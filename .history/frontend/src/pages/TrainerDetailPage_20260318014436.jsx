...
function buildRolesPayload(selectedCategories = [], selectedSpecialtiesByCategory = {}) {
  const payload = [];
...
function TrainerProfileForm({ profile, onAfterSave, pvDebug = false }) {
  ...
  const professionalDirty =
    dirtyMap.location || dirtyMap.experienceYears || dirtyMap.isOnline;
  ...
  const toggleSpecialty = useCallback((categoryValue, specialtyLabel) => {
    setForm((prev) => {
      ...
      const currentSpecs = prev.selectedSpecialtiesByCategory?.[categoryValue] || [];
            ...
            >
              <div className="space-y-5">
                ...
                >
                  ...
                  <p className="text-xs text-zinc-400">
                    Μπορείς να επιλέξεις περισσότερες από μία κατηγορίες.
                  </p>
                </div>
