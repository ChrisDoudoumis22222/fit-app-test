...
export default function AuthPage() {
  ...
  const redirectAuthenticatedUser = useCallback(
    async (
      session,
      { force = false, successToast = null, intent = null } = {}
    ) => {
  ...
  const handleLoginSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        const ensuredProfile = await safeEnsureUserProfile(user || sess?.user);
        const refreshedProfile = await refreshProfile().catch(() => null);
        const finalProfile = refreshedProfile || ensuredProfile || null;

        const nextRoute = getPostLoginRoute(user || sess?.user, finalProfile);

        redirectHandledUserIdRef.current = (user || sess?.user)?.id || null;
        autoRedirectKeyRef.current = "";
    ...
    async (e) => {
      ...
      try {
        ...
        if (activeSession?.user) {
          ...
          const ensuredProfile = await safeEnsureUserProfile(
            sess.user,
            signupIntent
          );
          const refreshedProfile = await refreshProfile().catch(() => null);
          ...
          const nextRoute = getPostLoginRoute(sess.user, finalProfile);
