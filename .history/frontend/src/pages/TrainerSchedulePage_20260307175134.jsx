...
function PremiumCard({
  ...
  return (
    <motion.div
      ...
      className={[
        "w-full h-full flex flex-col group relative overflow-hidden transition-all duration-300",
        "-mx-3 px-3 rounded-none border-0 bg-transparent shadow-none",
        "sm:mx-0 sm:px-0 sm:rounded-3xl sm:border sm:bg-gradient-to-b sm:from-zinc-900/60 sm:to-black/60 sm:backdrop-blur-xl sm:shadow-2xl",
...
function PremiumSwitch({ checked, onChange, label, dirty = false, disabled = false }) {
  return (
    <div className={["flex items-center gap-3", dirty ? "halo p-2 -m-2 rounded-full" : ""].join(" ")}>
      <button
        ...
        className={cn(
          "relative inline-flex h-7 w-12 items-center rounded-full transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-white/20 border shadow-[inset_0_1px_1px_rgba(255,255,255,.08)]",
          checked
            ? "bg-emerald-500 border-emerald-400/50 shadow-[0_8px_22px_rgba(16,185,129,.35)]"
      ...
      >
        <span
          className={cn(
            "absolute top-1 inline-block h-5 w-5 rounded-full bg-white transition-all duration-200 shadow-[0_3px_10px_rgba(0,0,0,.25)]",
...
export default function TrainerSchedulePage() {
  ...
  }, [performPendingNav, somethingDirty]); // intentionally using latest dirty state after save cycle

  useEffect(() => {
    if (!savingAll && leaveOpen && !somethingDirty && pendingNavRef.current) {
      setLeaveOpen(false);
      setTimeout(() => performPendingNav(), 0);
    }
  }, [savingAll, leaveOpen, somethingDirty, performPendingNav]);
