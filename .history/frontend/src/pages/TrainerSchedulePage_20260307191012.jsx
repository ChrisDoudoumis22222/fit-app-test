  ...
  return (
    ...
    >
      ...
      {(title || subtitle || icon || action) && (
        <div className="relative mb-5 flex w-full items-center justify-between gap-3 pl-1 sm:pl-0 text-left">
...
function TimePickerInput({
  ...
  return (
    <div className={cn("space-y-2", dirty ? "halo p-2 -m-2 rounded-2xl" : "")}>
      <PickerLabel label={label} icon={icon} />
      ...
      <div className="relative z-[30]" ref={wrapRef}>
        ...
        >
          ...
          <span className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-white/80 shrink-0">
        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              className={cn(
                "absolute mt-2 overflow-hidden rounded-3xl border border-white/10 bg-zinc-950/98 backdrop-blur-2xl shadow-[0_24px_80px_rgba(0,0,0,.55)]",
                isMobile ? "left-0 right-0" : "left-0 right-0 min-w-[320px]"
              )}
            >
              <div className="grid grid-cols-2 border-b border-white/10">
                <div className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.16em] text-white/45 text-center">
                  Ώρα
                </div>
                <div className="px-4 py-3 text-xs font-semibold uppercase tracking-[0.16em] text-white/45 text-center border-l border-white/10">
                  Λεπτά
                </div>
              </div>
              ...
              <div className="grid grid-cols-2">
                ...
                <div className="max-h-56 overflow-y-auto p-2 space-y-1 border-l border-white/10">
                  {minutes.map((m) => {
    ...
    (async () => {
      try {
        ...
        if (Array.isArray(availData) && availData.length > 0) {
          ...
          const minStart =
            availData.map((r) => String(r.start_time).slice(0, 5)).sort()[0] ||
            "08:00";
          const maxEnd =
            availData
              .map((r) => String(r.end_time).slice(0, 5))
              .sort()
              .slice(-1)[0] || "21:00";
    ...
    try {
      ...
      const conflict = (dateStr, sT, eT) => {
        ...
        return (bookings || [])
          ...
          .some((b) => {
            const bs =
              toMinutesLoc(String(b.start_time).slice(0, 5)) -
              (Number(b.break_before_min) || 0);
            const be =
              toMinutesLoc(String(b.end_time).slice(0, 5)) +
      ...
      for (let i = 0; i < horizonDays; i++) {
        ...
        for (const b of todaysBlocks) {
          let cur = toMinutesLoc(String(b.start_time).slice(0, 5));
          const endM = toMinutesLoc(String(b.end_time).slice(0, 5));
