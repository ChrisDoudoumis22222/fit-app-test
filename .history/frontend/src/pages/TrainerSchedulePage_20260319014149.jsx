...
import AvalabilityModalScheduler from "../components/avalabilitymodalscheduler";
...
function useFloatingPanel(triggerRef, open, { width = 340, gap = 10 } = {}) {
...
function TimePickerInput({
  ...
  const panelStyle = useFloatingPanel(triggerRef, open, {
    width: isMobile ? window?.innerWidth - 16 || 360 : 380,
    gap: 10,
  });
...
function DatePickerInput({
  ...
  const isMobile = useIsMobile(640);
  const panelStyle = useFloatingPanel(triggerRef, open, {
    width: isMobile ? window?.innerWidth - 16 || 360 : 360,
    gap: 10,
  });
            ...
            {hasTrainer ? (
              <div className="space-y-6 sm:space-y-8">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-6 items-stretch">
                  <LazySection>
                    <AvalabilityModalScheduler
                      ALL_DAYS={ALL_DAYS}
                      selectedDays={selectedDays}
                      setSelectedDays={setSelectedDays}
                      selectedDaysDirty={selectedDaysDirty}
                      startTime={startTime}
                      setStartTime={setStartTime}
                      startTimeDirty={startTimeDirty}
                      endTime={endTime}
                      setEndTime={setEndTime}
                      endTimeDirty={endTimeDirty}
                      isOnline={isOnline}
                      setIsOnline={setIsOnline}
                      isOnlineDirty={isOnlineDirty}
