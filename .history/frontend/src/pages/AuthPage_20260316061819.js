  ...
  const redirectAuthenticatedUser = useCallback(
    async (
      ...
      try {
        ...
        const profile = await safeEnsureUserProfile(session.user, intent);
  ...
  const handleLoginSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        const profile = await safeEnsureUserProfile(data?.user || sess?.user);
        await refreshProfile().catch(console.error);
    ...
    async (e) => {
      ...
      try {
        ...
        if (activeSession?.user) {
          ...
          const profile = await safeEnsureUserProfile(sess.user, signupIntent);
          await refreshProfile().catch(console.error);
