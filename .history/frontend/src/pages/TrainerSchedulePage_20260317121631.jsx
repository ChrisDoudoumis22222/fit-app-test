...
const WEEKDAYS = [
  { value: "monday", label: "Δευ" },
  { value: "tuesday", label: "Τρι" },
  { value: "wednesday", label: "Τετ" },
  { value: "thursday", label: "Πεμ" },
  { value: "friday", label: "Παρ" },
  { value: "saturday", label: "Σαβ" },
  { value: "sunday", label: "Κυρ" },
];
...
function deriveUiStateFromForm(form) {
  ...
  const selectedDays = WEEKDAYS.filter(
    (day) =>
      Array.isArray(availabilityByDay[day.value]) &&
  ...
  return {
    ...
    isOnline:
      typeof firstSlot?.is_online === "boolean"
        ? !!firstSlot.is_online
        : !!form?.is_online,
  };
}
...
export default function TrainerAvailabilityStep({
  ...
  const commitWithLegacyCallbacks = (
    nextDays,
    nextStart,
    nextEnd,
    nextOnline
