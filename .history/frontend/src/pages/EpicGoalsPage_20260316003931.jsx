    ...
    return (
      <>
        ...
        <div className="min-h-[var(--vh-unit)] bg-gradient-to-br from-black via-zinc-900 to-black relative text-white flex items-center justify-center px-4">
          <ZincBackground />
          <p className="relative z-10 text-center text-zinc-400">
        ...
        <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-10 space-y-8 pb-[calc(env(safe-area-inset-bottom,0px)+4.5rem)] sm:pb-10">
          ...
          >
            <motion.button
              ...
              className={cn(
                "inline-flex items-center gap-2 sm:gap-3 px-6 sm:px-8 py-3 sm:py-4",
                "text-white text-sm sm:text-base font-semibold rounded-2xl shadow-lg transition-all duration-300",
                loading || !profileLoaded
                  ? "bg-zinc-800/80 cursor-not-allowed opacity-65"
          ...
          {!loading && profileLoaded && goals.length === 0 && (
            <motion.div
...
function StatCardSkeleton() {
  return (
    <div className="relative overflow-hidden rounded-2xl sm:rounded-3xl bg-black/40 backdrop-blur-xl border border-zinc-800/50 p-4 sm:p-6 shadow-[0_20px_50px_rgba(0,0,0,0.35)]">
      <div className="absolute inset-0 bg-gradient-to-br from-zinc-700/10 via-transparent to-transparent" />
      <div className="relative animate-pulse">
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-lg bg-zinc-700/70" />
          <div className="w-12 sm:w-16 h-7 sm:h-8 rounded-lg bg-zinc-700/70" />
        </div>
        <div className="w-24 sm:w-32 h-4 rounded-lg bg-zinc-800/80" />
      </div>
    </div>
  );
}