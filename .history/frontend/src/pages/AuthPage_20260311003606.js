...
export default function AuthPage() {
  ...
  const persistProfile = useCallback(async (userId) => {
    if (!userId) return null;

    try {
      const { data, error: profErr } = await withTimeout(
      ...
      safeSetStorage("pv_profile", data || null);
      return data || null;
    } catch (err) {
  ...
  const ensureUserProfile = useCallback(
    async (authUser, intent = null) => {
      ...
      const isGoogleSignupUser = intent?.type === "google_signup_user";
      ...
      const payload = {
        ...
        avatar_url:
          ...
          null,
        role: isGoogleSignupUser
          ? "user"
          : authUser.user_metadata?.role ||
            authUser.app_metadata?.role ||
            "user",
        specialty: isGoogleSignupUser
          ? null
          : authUser.user_metadata?.specialty || null,
        roles:
          isGoogleSignupUser
            ? []
        ...
        location:
          (isGoogleSignupUser
            ? intent?.location || "Όλες οι πόλεις"
            : authUser.user_metadata?.location) || "Όλες οι πόλεις",
  ...
  const redirectAuthenticatedUser = useCallback(
    ...
    [ensureUserProfile, getPostLoginRoute, navigate, persistSession, pushToast, refreshProfile]
  );
  ...
  useEffect(() => {
    ...
    const bootAuthenticatedRedirect = async () => {
      const oauthPending =
        safeSessionGet(OAUTH_PENDING_KEY) === "1" || hasOAuthParams();
      ...
      try {
        ...
        if (data?.session?.user) {
          await redirectAuthenticatedUser(data.session);
        }
      } catch (err) {
        console.error("boot authenticated redirect error:", err);
      }
    };

    void bootAuthenticatedRedirect();
    ...
    const {
      ...
      if (event === "SIGNED_IN" && session?.user) {
        ...
        if (oauthPending) {
          ...
          void redirectAuthenticatedUser(session, { force: true });
        }
      }
  ...
  }, [finalizeOAuthLogin, redirectAuthenticatedUser]);

  const startGoogleOAuth = async (intent) => {
  ...
  const submit = async (e) => {
    ...
    try {
      if (mode === "login") {
        ...
        if (data?.user?.id) {
          const { error: profileUpsertErr } = await withTimeout(
        ...
        if (activeSession?.user) {
          ...
          const profile = await ensureUserProfile(sess.user);
          await refreshProfile().catch(console.error);
