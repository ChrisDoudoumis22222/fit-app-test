...
export default function TrainerDashboard() {
  ...
  const onDiplomaSelect = async (e) => {
    ...
    // build a path under the user's UID folder
    const path = `${profile.id}/${Date.now()}.${file.name.split(".").pop()}`;
    const { error: upError } = await supabase.storage
      .from("diplomas")        // this must match your bucket name exactly
      .upload(path, file, { cacheControl: "3600", upsert: true });

    if (upError) {
      // If the bucket doesn't exist, upError.message often contains "Bucket not found"
      if (upError.message?.toLowerCase().includes("bucket not found")) {
        return setUpErr(
          "Storage bucket “diplomas” not found. Please create a bucket named “diplomas” in Supabase → Storage."
        );
      }
      return setUpErr(upError.message);
    }

    // get public URL of the uploaded diploma
    const { data } = await supabase.storage
      .from("diplomas")
      .getPublicUrl(path);

    // update the profile row with diploma_url
    await supabase
      .from("profiles")
  ...
  return (
    <>
      {/* TrainerMenu no longer needs a prop; it disables "My services" internally */}
      <TrainerMenu />
