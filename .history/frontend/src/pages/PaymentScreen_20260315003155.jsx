...
function groupHasVisibleRows(group, statusFilter) {
  if (statusFilter === "all") return group.rows.length > 0;
  return group.rows.some((r) => normalizeStatus(r.status) === statusFilter);
}
...
const BaseBackground = () => (
  <>
    ...
    <div className="fixed inset-0 -z-40 pointer-events-none opacity-30 bg-[radial-gradient(circle_at_50%_20%,rgba(255,255,255,0.12),transparent_55%)]" />
    <div className="fixed inset-0 -z-40 pointer-events-none opacity-20 bg-[linear-gradient(to_right,rgba(255,255,255,0.05)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,0.05)_1px,transparent_1px)] bg-[size:24px_24px]" />
  </>
);
  ...
  return (
    <Glass className="overflow-hidden">
      <button
        ...
        className="flex w-full items-center gap-3 p-4 text-left transition hover:bg-white/[.03] sm:p-6"
      >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <Chip tone="zinc" className="hidden sm:inline-flex">
              {visibleCount} εμφανίζονται
            </Chip>
          </div>

          <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-white/60 sm:text-sm">
            <span>
            ...
            <span>
              Σύνολο κρατήσεων:{" "}
              <span className="font-semibold text-white/80">{group.rows.length}</span>
            </span>
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            <Chip tone="amber">{buckets.pending.length} σε αναμονή</Chip>
            <Chip tone="emerald">{buckets.accepted.length} εγκεκριμένες</Chip>
            <Chip tone="rose">{buckets.declined.length} απορριφθείσες</Chip>
          </div>
        </div>

        <div className="shrink-0 rounded-xl border border-white/10 bg-white/5 p-2">
          {expanded ? (
            <ChevronUp className="h-5 w-5 text-white/75" />
          ) : (
...
export default function PaymentScreen() {
  ...
  const fetchBookings = useCallback(async (userId, take, trainerMode) => {
    ...
    const baseFields =
      "id,trainer_id,user_id,date,start_time,end_time,duration_min,status,created_at,is_online,user_name,user_email,trainer_name,amount,currency_code,payment_method,payment_status,paid_at,payment_reference,note,sale,updated_at";

    const selectWithEmails =
      `${baseFields},` +
      "trainer:profiles!trainer_bookings_trainer_id_fkey(id,full_name,avatar_url,email)," +
      "user:profiles!trainer_bookings_user_id_fkey(id,full_name,avatar_url,email)";

    const selectWithoutEmails =
      `${baseFields},` +
    ...
    const runQuery = async (selectString) => {
      let query = supabase.from("trainer_bookings").select(selectString);
      ...
      return withTimeout(query, 12000, "Timeout φόρτωσης κρατήσεων");
    };

    try {
      let response = await runQuery(selectWithEmails);
      ...
      if (response?.error) {
        ...
        if (shouldFallback) {
          response = await runQuery(selectWithoutEmails);
        }
      }
  ...
  const prepared = useMemo(() => {
    return rows.map((b) => {
      const trainerEmail = b?.trainer?.email || b?.trainer_email || null;
      const userEmail = b?.user?.email || b?.user_email || null;
  ...
  const groupedAll = useMemo(() => {
    ...
    for (const b of prepared) {
      ...
      const groupKey =
        cp.id ||
        cp.email ||
        `${safeLower(cp.name)}-${safeLower(b.user_name)}-${safeLower(b.trainer_name)}-${b.id}`;
      ...
      if (!group.email && cp.email) group.email = cp.email;
      if (!group.avatar && cp.avatar) group.avatar = cp.avatar;
      if ((!group.name || group.name === "—") && cp.name) group.name = cp.name;
    }
    ...
    for (const g of arr) {
      ...
      g.searchBlob = buildSearchBlob([
        g.name,
        g.email,
        ...g.rows.flatMap((row) => [
          row?.counterparty?.name,
          row?.counterparty?.email,
          row?.user_name,
          row?.resolved_user_email,
          row?.trainer_name,
          row?.trainer_email,
          row?.user?.full_name,
          row?.user?.email,
  ...
  const grouped = useMemo(() => {
    ...
    return groupedAll.filter((group) => {
      const searchOk = !needle || group.searchBlob.includes(needle);
      const statusOk = groupHasVisibleRows(group, statusFilter);
      return searchOk && statusOk;
    });
  }, [groupedAll, q, statusFilter]);
  ...
  const expandAll = () => {
    const next = {};
    for (const g of grouped) next[g.id] = false;
        ...
        >
          <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h1 className="text-[28px] leading-tight font-extrabold tracking-tight text-white sm:text-4xl">
                  {title}
                </h1>
            ...
            <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
              <button
                ...
                className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-white/10 bg-[#0f1115] px-3 text-white hover:bg-[#171a20]"
                title="Ανανέωση"
              >
              ...
              <div className="relative flex-1 sm:flex-none">
                ...
                <input
                  ...
                  placeholder="Αναζήτηση πελάτη / trainer (όνομα ή email)"
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <button
              type="button"
              onClick={expandAll}
              className="inline-flex h-10 items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 text-sm font-semibold text-white hover:bg-white/10"
            >
            ...
            <div className="text-xs text-white/45 sm:text-sm">
              Εμφανίζονται <span className="font-semibold text-white/75">{grouped.length}</span>{" "}
              {grouped.length === 1 ? "group" : "groups"}
            </div>
          </div>
        </motion.div>
            ...
            <div className="flex flex-col items-center gap-4 py-20 text-zinc-300">
              ...
              <div className="space-y-1 text-center">
                <p className="text-base font-semibold text-white">
                  {q.trim() ? "Δεν βρέθηκαν πελάτες / trainers" : "Δεν βρέθηκαν κρατήσεις"}
                </p>

                <p className="max-w-md text-sm text-white/65">
                  {q.trim()
                    ? "Δοκίμασε άλλο όνομα ή email."
