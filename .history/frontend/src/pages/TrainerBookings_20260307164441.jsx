...
import BookingDetailsDock from "../components/BookingDetailsDock";
...
function MonthMiniChip({ booking, onOpen }) {
  ...
  return (
    ...
    >
      <span
        className={[
          "block w-full rounded-md border px-1 py-[2px]",
          "text-[9px] leading-none font-medium truncate",
          tone,
        ].join(" ")}
      >
...
function NewBookingsPopup({ trainerId, onOpenDetails, refreshKey }) {
  ...
  useEffect(() => {
    ...
    const base = `rt-trainer-bookings-${trainerId}`;
    const unique =
      `${base}-` +
      `${Date.now()}-` +
    ...
    try {
      const chans =
        ...
        (supabase.realtime &&
          typeof supabase.realtime.getChannels === "function" &&
          supabase.realtime.getChannels()) ||
        [];

      for (const ch of chans) {
        const topic = ch?.topic || "";
        if (topic.includes(base)) {
          try {
            ch.unsubscribe?.();
          } catch {}
          try {
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        exit={{ opacity: 0, y: -8 }}
        transition={{ duration: 0.22, ease: [0.22, 1, 0.36, 1] }}
        className="mx-0 sm:mx-0 -mt-4 sm:-mt-1 md:mt-4"
      >
        <Glass className="p-3 sm:p-4">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            ...
            {!loading &&
              items.map((b) => (
                <div key={b.id} className="min-w-[180px] sm:min-w-[240px]">
                  <BookingPill
                    b={b}
...
function WeeklyScheduleGrid({
  ...
  const content = (
    <>
      ...
      <div className={isMobile ? "overflow-x-auto -mx-1.5 px-1.5" : "overflow-x-auto"}>
        <div
          className={
            isMobile
    ...
    <p className="mt-1.5 text-[12px] sm:text-base text-white/70 leading-[1.3] max-w-[260px] sm:max-w-none">
      Έχετε πλήρη εικόνα για τις κρατήσεις και το πρόγραμμά σας
          ...
          >
            ...
            >
              <div
                className={`grid grid-cols-1 lg:grid-cols-[1fr,420px] items-start ${
              ...
              >
                ...
                <div className="self-stretch min-h-full px-1.5 sm:px-0">
