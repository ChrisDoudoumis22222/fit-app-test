...
const Glass = ({ className = "", children }) => (
  <div
    className={[
      "relative overflow-hidden rounded-[24px] sm:rounded-[28px] border border-white/10",
      "bg-[rgba(17,18,21,.68)] backdrop-blur-xl",
      "shadow-[inset_0_1px_0_rgba(255,255,255,.04),0_10px_30px_rgba(0,0,0,.45)]",
      className,
    ].join(" ")}
  >
    <div className="pointer-events-none absolute inset-0 rounded-[24px] sm:rounded-[28px] overflow-hidden">
...
function Button({
  ...
  const base =
    "inline-flex items-center justify-center rounded-xl font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-white/20 disabled:opacity-60 disabled:cursor-not-allowed";

  const sizes = {
    sm: "h-9 px-3 text-sm",
    md: "h-10 px-4 text-sm",
    lg: "h-11 sm:h-12 px-4 sm:px-5 text-sm sm:text-base",
  };
...
const Li = ({ icon: Icon, label, value }) => (
  <li className="flex items-start gap-3 rounded-2xl border border-white/6 bg-white/[0.02] px-3 py-3 sm:px-4">
    <div className="mt-0.5 shrink-0 rounded-xl border border-white/8 bg-white/[0.04] p-2">
      <Icon className="h-4 w-4 sm:h-5 sm:w-5 text-white/70" />
    </div>
    <div className="min-w-0 flex-1">
      <p className="text-[11px] sm:text-xs uppercase tracking-wide text-white/45">
        {label}
      </p>
      <p className="mt-1 text-sm sm:text-[15px] leading-relaxed text-gray-200 break-words">
        {value || "—"}
      </p>
    </div>
  </li>
...
export default function BookingDetailsDock({
  ...
  const showCalendarButton =
    !!googleCalendarUrl && status !== "pending" && status !== "declined";

  const statusTone =
    {
      pending: "bg-amber-500/20 text-amber-300 border border-amber-400/10",
      accepted:
        "bg-emerald-600/20 text-emerald-300 border border-emerald-400/10",
      declined: "bg-rose-600/20 text-rose-300 border border-rose-400/10",
      cancelled: "bg-rose-600/20 text-rose-300 border border-rose-400/10",
    }[status] || "bg-amber-500/20 text-amber-300 border border-amber-400/10";
  ...
  return (
    <Glass className="w-full p-4 sm:p-5 lg:p-6 h-fit">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs sm:text-sm text-white/45">
            Πλήρης εικόνα της κράτησης σε mobile και desktop.
          </p>
        </div>
        ...
        {onClose && (
          <button
            onClick={onClose}
            aria-label="Κλείσιμο"
            className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] text-white/65 transition hover:bg-white/[0.08] hover:text-white shrink-0"
          >
            <X className="h-5 w-5" />
          </button>
        )}
      ...
      {!id ? null : loading ? (
        <div className="animate-pulse space-y-3">
          ...
          <div className="h-32 bg-white/[.06] rounded-2xl border border-white/10" />
          <div className="h-6 bg-white/[.06] rounded border border-white/10" />
          <div className="h-6 bg-white/[.06] rounded border border-white/10" />
        </div>
      ) : err ? (
        <div className="rounded-2xl border border-red-400/10 bg-red-900/30 p-3 text-sm text-red-300">
          {err}
        </div>
      ) : (
        <>
          <div className="mb-5 sm:mb-6 flex flex-col sm:flex-row sm:items-center gap-4">
              ...
              {clientAvatar ? (
                <div className="relative shrink-0">
                  <img
                    ...
                    className="h-14 w-14 sm:h-16 sm:w-16 rounded-full object-cover border border-white/15 ring-2 ring-white/10 shadow-[0_8px_24px_rgba(0,0,0,.35)]"
                  />
                </div>
              ) : (
                <div className="grid h-14 w-14 sm:h-16 sm:w-16 place-items-center rounded-full bg-white/8 border border-white/10 ring-2 ring-white/10 shadow-[0_8px_24px_rgba(0,0,0,.35)] shrink-0">
                  <User className="h-6 w-6 sm:h-7 sm:w-7 text-white/70" />
                </div>
              )}

              <div className="min-w-0">
                <div className="text-lg sm:text-xl font-medium text-white truncate">
                  {full?.client?.full_name || full?.user_name || "Χρήστης"}
                </div>

                <div className="mt-2 flex flex-wrap items-center gap-2">
                  <span
                    className={`inline-flex items-center rounded-xl px-2.5 py-1 text-xs font-medium ${statusTone}`}
                  >
                    {statusLabel}
                  </span>

                  <span className="inline-flex items-center rounded-xl border border-white/10 bg-white/[0.04] px-2.5 py-1 text-xs text-white/60">
          ...
          <div className="mt-6 flex flex-col-reverse sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-3 justify-end">
            {status === "pending" && (
              <>
                <Button
                  variant="secondary"
            ...
            {showCalendarButton && (
              <Button
                ...
                className="w-full sm:w-auto gap-2.5 font-semibold"
                title="Ημερολόγιο"
              >
