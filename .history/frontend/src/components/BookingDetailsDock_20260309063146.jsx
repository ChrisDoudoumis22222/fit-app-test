...
const Glass = ({ className = "", children, accepted = false }) => (
  <motion.div
    ...
    className={[
      ...
      "bg-[rgba(12,13,16,.78)] backdrop-blur-xl",
      accepted ? "ring-1 ring-emerald-400/20" : "",
      className,
    ].join(" ")}
  >
    <div className="pointer-events-none absolute inset-0 rounded-3xl overflow-hidden">
      <motion.div
        initial={false}
        animate={{ opacity: accepted ? 0.85 : 0.28 }}
        transition={{ duration: 0.25, ease: "easeOut" }}
        className={
          accepted
...
function Button({
  ...
  const base =
    "inline-flex items-center justify-center rounded-2xl font-medium transition focus:outline-none focus:ring-2 focus:ring-white/15 disabled:opacity-60 disabled:cursor-not-allowed";
  ...
  const variants = {
    primary:
      "bg-emerald-600/90 hover:bg-emerald-600 text-white border border-emerald-400/15 shadow-[0_6px_18px_rgba(16,185,129,.22)]",
    secondary:
      "bg-white/[0.05] hover:bg-white/[0.08] text-white border border-white/8",
    ghost:
      "bg-transparent hover:bg-white/[0.05] text-white border border-white/8",
    calendar:
      "bg-white text-black border border-white/80 hover:bg-zinc-200 shadow-[0_10px_24px_rgba(255,255,255,.12)]",
  };
...
function InfoRow({ icon: Icon, label, value }) {
  return (
    <div className="flex items-start gap-3 rounded-2xl bg-white/[0.035] px-3 py-3">
      <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white/[0.04] border border-white/6">
      ...
      <div className="min-w-0">
        ...
        <div className="mt-1 break-words text-sm leading-relaxed text-white/90">
          {value || "—"}
        </div>
      </div>
    </div>
    ...
    <>
      ...
      >
        <div className="flex h-full w-full flex-col bg-[#090a0d]">
          <div className="sticky top-0 z-10 border-b border-white/6 bg-[#090a0d]/96 px-4 pb-3 pt-4 backdrop-blur">
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              {onClose && (
                <button
                  onClick={onClose}
                  aria-label="Κλείσιμο"
                  className="grid h-10 w-10 place-items-center rounded-full border border-white/8 bg-white/[0.04] text-white/70 transition hover:bg-white/10 hover:text-white"
                >
                  <X className="h-5 w-5" />
                </button>
              )}
...
export default function BookingDetailsDock({
  ...
  const statusTone =
    {
      pending: "bg-amber-500/14 text-amber-300 border border-amber-400/8",
      accepted: "bg-emerald-600/14 text-emerald-300 border border-emerald-400/8",
      declined: "bg-rose-600/14 text-rose-300 border border-rose-400/8",
      cancelled: "bg-rose-600/14 text-rose-300 border border-rose-400/8",
    }[status] || "bg-amber-500/14 text-amber-300 border border-amber-400/8";
      ...
      {!id ? null : loading ? (
        ...
        <>
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                <motion.span
                  initial={false}
                  animate={status === "accepted" ? { scale: [1, 1.05, 1] } : { scale: 1 }}
                  transition={{ duration: 0.25, ease: "easeOut" }}
          ...
          <div className="space-y-3">
            <InfoRow icon={CalendarClock} label="Ημερομηνία" value={fmtDate(full?.date)} />
            <InfoRow icon={Clock} label="Ώρες" value={`${hhmm(full?.start_time)} – ${hhmm(full?.end_time)}`} />
            <InfoRow icon={Clock} label="Διάρκεια" value={`${full?.duration_min ?? 0} λεπτά`} />
            <InfoRow icon={Wifi} label="Τύπος" value={full?.is_online ? "Online συνεδρία" : "Δια ζώσης"} />
            {!!full?.note && <InfoRow icon={MapPin} label="Σημείωση" value={full.note} />}
            <InfoRow icon={User} label="Πελάτης" value={full?.client?.full_name || full?.user_name || "—"} />
            {!!(full?.client?.email || full?.user_email) && (
              <InfoRow icon={Mail} label="Email πελάτη" value={full?.client?.email || full?.user_email} />
            )}
            {!!(full?.trainer_info?.full_name || full?.trainer_name) && (
              <InfoRow icon={User} label="Προπονητής" value={full?.trainer_info?.full_name || full?.trainer_name} />
            )}
            <InfoRow
              icon={CalendarClock}
              label="Υποβλήθηκε"
        ...
        <>
              ...
              {clientAvatar ? (
                <img
                  ...
                  className={`h-14 w-14 rounded-full object-cover border ${
                    status === "accepted"
                      ? "border-emerald-300/25 ring-2 ring-emerald-300/10"
                      : "border-white/10 ring-2 ring-white/8"
                  }`}
                />
              ) : (
                <div
                  className={`grid h-14 w-14 place-items-center rounded-full border ${
                    status === "accepted"
                      ? "border-emerald-300/20 bg-emerald-500/10 ring-2 ring-emerald-300/10"
                      : "border-white/10 bg-white/6 ring-2 ring-white/8"
                  }`}
                >
          ...
          <div className="rounded-2xl bg-white/[0.03]">
            ...
            >
              ...
              <div className="grid h-9 w-9 place-items-center rounded-full bg-white/[0.04] border border-white/6">
                {mobileExpanded ? (
