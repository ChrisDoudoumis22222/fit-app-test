...
function hasCertifications(form) {
  if (Array.isArray(form?.certifications)) {
    return form.certifications.some((item) => hasNonEmptyString(item));
  }

  if (typeof form?.certifications === "string") {
    return hasNonEmptyString(form.certifications);
  }
...
function getStepValidationError(currentStep, form) {
  ...
  if (currentStep.key === "credentials") {
    if (!hasCertifications(form)) {
      return "Συμπλήρωσε τουλάχιστον μία πιστοποίηση.";
    }
      ...
      },
{
  ...
  subtitle: "Πρόσθεσε μια καθαρή φωτογραφία προφίλ για να εμπνέεις εμπιστοσύνη",
  icon: Camera,
},
      {
        dbStep: 5,
