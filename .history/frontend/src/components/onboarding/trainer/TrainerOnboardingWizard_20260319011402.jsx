...
import TrainerAvatarUploader from "./steps/TrainerAvatarUploader";
...
function hasAvatar(form) {
  return !!(
    form?.avatar_file ||
    form?.avatar_ready ||
    hasNonEmptyString(form?.avatar_url) ||
    hasNonEmptyString(form?.avatar_preview_url) ||
    hasNonEmptyString(form?.avatar_path)
  );
}
...
function hasDiploma(form) {
  ...
  return hasFileObject || hasReadyFlag || hasUrl || hasPreviewUrl || hasName;
}
...
function StepRenderer({
  ...
  if (stepKey === "photoProfile") {
    return (
      <TrainerAvatarUploader
        form={form}
        setField={setField}
        setForm={setForm}
      />
...
export default function TrainerOnboardingWizard({
  ...
  const actionBusy = saving || isSubmittingFinal || isSavingStep;
  ...
  const persistProgress = useCallback(
    async (targetDbStep) => {
      if (typeof onSaveProgress !== "function") return;

      setIsSavingStep(true);

      try {
        const data = buildSubmitData(form);
        await onSaveProgress(data, targetDbStep);
      } finally {
  ...
  const handleGoToStep = useCallback(
    async (idx) => {
      ...
      try {
        if (targetStep) {
          await persistProgress(targetStep.dbStep);
        }
    ...
    [actionBusy, currentIndex, steps, persistProgress]
  );
  ...
  const handleBack = useCallback(async () => {
    ...
    try {
      if (previousStep) {
        await persistProgress(previousStep.dbStep);
      }
  ...
  }, [currentIndex, actionBusy, steps, persistProgress]);
  ...
  const handleNext = useCallback(async () => {
    ...
    try {
      if (nextStep) {
        await persistProgress(nextStep.dbStep);
      }
      ...
      <div className="relative min-h-[100svh] overflow-hidden bg-zinc-900 text-white">
        ...
        <div className="relative z-10 hidden min-h-[100svh] lg:block">
          <div className="w-full px-6 py-6 xl:px-8 xl:py-8">
            ...
            <div className="mt-6">
              <div className="rounded-3xl border border-zinc-700/50 bg-black/35 px-6 pb-6 pt-5 backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.35)]">
