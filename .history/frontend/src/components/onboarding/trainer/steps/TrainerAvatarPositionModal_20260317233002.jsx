          ...
          <div className="space-y-4">
            ...
            >
              ...
              {showPreview && imageUrl ? (
                <div className="absolute right-4 top-4 z-10 rounded-[22px] border border-white/10 bg-black/75 p-3 backdrop-blur-md shadow-[0_20px_40px_rgba(0,0,0,0.35)]">
                  <div className="mb-2 text-xs font-medium text-white">
                    Preview
                  </div>

                  <div className="relative h-24 w-24 overflow-hidden rounded-full border-2 border-white/15 bg-black">
                    <img
                      src={imageUrl}
          ...
          <div className="flex flex-col gap-4">
            <button
              ...
              className={`inline-flex w-full items-center justify-center gap-2 rounded-2xl px-4 py-3 text-sm font-medium transition ${
                imageUrl
                  ? "border border-white/10 bg-white/5 text-zinc-200 hover:border-white/20 hover:bg-white/10 hover:text-white"
                  : "cursor-not-allowed border border-white/5 bg-white/[0.03] text-zinc-600 opacity-60"
              }`}
            >
              {showPreview ? (
                <>
            ...
            <div className="rounded-[24px] border border-white/10 bg-white/[0.03] p-4">
              <div className="space-y-2 text-xs text-zinc-400">
                <div className="flex items-center justify-between">
                  <span>Οριζόντια</span>
                  <span className="text-white">{Math.round(posX)}%</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>Κάθετα</span>
                  <span className="text-white">{Math.round(posY)}%</span>
                </div>
                <div className="flex items-center justify-between">
                  <span>Zoom</span>
                  <span className="text-white">{zoom.toFixed(2)}x</span>
                </div>
              </div>
            </div>
