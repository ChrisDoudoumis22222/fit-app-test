  ...
  return (
    <div ref={menuRef} className="relative">
      <button
        type="button"
        onClick={onToggle}
        className="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-white/10 bg-black/45 text-white backdrop-blur-md transition hover:border-white/20 hover:bg-black/55"
...
function EmptyPicker({ busy, onUploadClick, onCameraClick }) {
  return (
    <div className="rounded-[28px] border border-dashed border-zinc-700 bg-zinc-950/80 p-5 md:p-6">
      <div className="flex flex-col items-center justify-center text-center">
        ...
        <p className="mt-2 max-w-[420px] text-sm text-zinc-400">
          Επίλεξε εικόνα από τη συσκευή σου ή άνοιξε την κάμερα. Μετά το upload
          θα μπορείς να ρυθμίσεις και τη θέση της.
        </p>

        <div className="mt-5 grid w-full max-w-[460px] grid-cols-1 gap-2.5 sm:grid-cols-2">
          <button
            type="button"
            onClick={onUploadClick}
            disabled={busy}
            className={cn(
              "inline-flex h-12 items-center justify-center gap-2 rounded-2xl border border-zinc-700 bg-zinc-950 px-4 text-sm font-semibold text-white transition",
              "hover:border-white/25 hover:bg-zinc-900",
              "disabled:cursor-not-allowed disabled:opacity-60"
            )}
          >
            <Upload className="h-4 w-4" />
            {busy ? "Γίνεται επεξεργασία..." : "Επιλογή εικόνας"}
          </button>
          ...
          <div className="mb-4 flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-sm text-zinc-400">
                Ανέβασε μια καθαρή φωτογραφία προφίλ
              </p>
            </div>
          </div>
