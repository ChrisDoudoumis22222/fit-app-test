...
const overlapsCandidateWithBooking = (
  ...
  const cs = timeToMinutes(hhmm(candidateStart));
  const ce = timeToMinutes(hhmm(candidateEnd));
  const bs = timeToMinutes(hhmm(bookingStart)) - Math.max(0, breakBeforeMin);
  const be = timeToMinutes(hhmm(bookingEnd)) + Math.max(0, breakAfterMin);

  if (![cs, ce, bs, be].every(Number.isFinite)) return false;
  return cs < be && ce > bs;
};
...
type BookingConflictRow = {
  ...
  start_time: string;
  end_time: string;
  break_before_min?: number | null;
  break_after_min?: number | null;
  status?: string | null;
};
...
function buildDerivedSlotsForRange({
  rangeStartISO,
  rangeEndISO,
  availabilityRows,
  holidayRows,
  ...
  bookingRows: BookingConflictRow[];
  slotMinutes: number;
  breakMinutes: number;
}) {
  const rows: SlotRow[] = [];
  ...
  const slotMin = clamp(Number(slotMinutes || 60), 15, 240);
  const breakMin = clamp(Number(breakMinutes || 0), 0, 60);
  const step = slotMin + breakMin;

  const bookingsByDate = new Map<string, BookingConflictRow[]>();
  for (const b of bookingRows || []) {
    if (!bookingsByDate.has(b.date)) bookingsByDate.set(b.date, []);
    bookingsByDate.get(b.date)!.push({
      ...b,
  ...
  for (const a of availabilityRows || []) {
    a.start_time = hhmm(a.start_time);
  ...
  for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
    ...
    const isHoliday = (holidayRows || []).some((h) =>
      withinDate(iso, h.starts_on, h.ends_on)
    );
    if (isHoliday) continue;

    const weekdayKey = weekdayNames[d.getDay()];
    const dayWindows = (availabilityRows || []).filter(
      (a) => String(a.weekday || "").toLowerCase() === weekdayKey
    );
    ...
    for (const win of dayWindows) {
      let cur = timeToMinutes(hhmm(win.start_time));
      const endMin = timeToMinutes(hhmm(win.end_time));

      if (!Number.isFinite(cur) || !Number.isFinite(endMin)) continue;
      if (endMin <= cur) continue;

      while (cur + slotMin <= endMin) {
        const slotStart = minutesToHHMM(cur);
        const slotEnd = minutesToHHMM(cur + slotMin);

        const conflict = dayBookings.some((b) =>
          overlapsCandidateWithBooking(
            slotStart,
            slotEnd,
            b.start_time,
            b.end_time,
            Number(b.break_before_min || 0),
            Number(b.break_after_min || 0)
          )
        );

        const key = `${iso}|${slotStart}|${slotEnd}`;

        if (!conflict && !seen.has(key)) {
          seen.add(key);
          rows.push({
            date: iso,
            start_time: slotStart,
            end_time: slotEnd,
            is_online: !!win.is_online,
            status: "open",
          });
        }
...
export default function QuickBookModal({
  ...
  const loadAvailability = useCallback(async () => {
    ...
    try {
      ...
      const [
        { data: bookedData, error: bookedError },
        { data: avData, error: avError },
        { data: holData, error: holError },
      ...
      const slot_minutes = clamp(
        Number(settingsData?.slot_minutes ?? 60),
        15,
        240
      );
      const break_minutes = clamp(
        Number(settingsData?.break_minutes ?? 0),
        0,
        60
      );

      setScheduleSettings({ slot_minutes, break_minutes });

      const availabilityRows = (avData || []) as AvailabilityRow[];
      const holidayRows = (holData || []) as HolidayRow[];
      const bookingRows = (bookedData || []) as BookingConflictRow[];

      const derived = buildDerivedSlotsForRange({
        rangeStartISO,
        rangeEndISO,
        availabilityRows,
        holidayRows,
      ...
      setOpenRows(derived);
      setShowNoScheduleToast(availabilityRows.length === 0);
    } catch (e: any) {
      console.error(e);
      setBanner({
        type: "error",
  ...
  const submitBooking = async () => {
    ...
    try {
      ...
      const freshSlots = buildDerivedSlotsForRange({
        rangeStartISO: selectedSlot.date,
        rangeEndISO: selectedSlot.date,
        availabilityRows: (avData || []) as AvailabilityRow[],
        holidayRows: (holData || []) as HolidayRow[],
        bookingRows: (bookedData || []) as BookingConflictRow[],
        slotMinutes: slot_minutes,
        breakMinutes: break_minutes,
      });
      ...
      if (!stillAvailable) {
        ...
        setBanner({
          type: "error",
          title: "Η ώρα δεν είναι πλέον διαθέσιμη",
          message:
            "Το συγκεκριμένο slot μόλις έκλεισε ή άλλαξε το πρόγραμμα του προπονητή. Διάλεξε άλλη ώρα.",
        });
        goBack("time");
        return;
      }
      ...
      const payload = {
        ...
        break_before_min: 0,
        break_after_min: break_minutes,
        note: note.trim() || null,
        status: "pending",
        is_online: !!stillAvailable.is_online,
