...
const Li = ({ icon: Icon, label, value, mobileSimple = false }) => (
  <li
    className={
      mobileSimple
        ? "rounded-2xl border border-white/8 bg-white/[0.03] px-3 py-3"
        : "flex items-start gap-3"
    }
  >
    {mobileSimple ? (
      <div className="flex items-start gap-3">
        <div className="mt-0.5 grid h-9 w-9 place-items-center rounded-xl bg-white/[0.04] border border-white/8 shrink-0">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 break-words text-sm leading-relaxed text-white/88">
            {value || "—"}
          </p>
        </div>
      </div>
  ...
  return (
    <>
      <motion.div
        ...
        className="fixed inset-0 z-[90] bg-black/70 backdrop-blur-sm sm:hidden"
      />
      ...
      >
        <div className="flex h-full w-full flex-col bg-[#0b0c0f]">
          <div className="sticky top-0 z-10 border-b border-white/8 bg-[#0b0c0f]/95 px-4 pb-3 pt-4 backdrop-blur">
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              {onClose && (
                <button
                  onClick={onClose}
                  aria-label="Κλείσιμο"
                  className="grid h-10 w-10 place-items-center rounded-full border border-white/10 bg-white/[0.04] text-white/70 transition hover:bg-white/10 hover:text-white"
                >
                  <X className="h-5 w-5" />
                </button>
              )}
...
export default function BookingDetailsDock({
  ...
  const statusTone =
    {
      pending: "bg-amber-500/18 text-amber-300 border border-amber-400/10",
      accepted: "bg-emerald-600/18 text-emerald-300 border border-emerald-400/10",
      declined: "bg-rose-600/18 text-rose-300 border border-rose-400/10",
      cancelled: "bg-rose-600/18 text-rose-300 border border-rose-400/10",
    }[status] || "bg-amber-500/18 text-amber-300 border border-amber-400/10";
    ...
    <div className="space-y-4">
      {!id ? null : loading ? (
        <div className="space-y-3 animate-pulse">
          <div className="h-24 rounded-3xl border border-white/10 bg-white/[.05]" />
          <div className="h-16 rounded-2xl border border-white/10 bg-white/[.05]" />
          <div className="h-16 rounded-2xl border border-white/10 bg-white/[.05]" />
          <div className="h-16 rounded-2xl border border-white/10 bg-white/[.05]" />
        </div>
      ) : err ? (
        ...
        <>
          ...
          <div className="sticky bottom-0 -mx-1 mt-5 bg-gradient-to-t from-[#0b0c0f] via-[#0b0c0f]/95 to-transparent px-1 pb-1 pt-4">
            <div className="space-y-3">
              {showCalendarButton && (
                <Button
                  variant="calendar"
