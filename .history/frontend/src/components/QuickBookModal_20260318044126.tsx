...
export default function QuickBookModal({
  ...
  const resolvePricingTrainerId = useCallback(async () => {
    const directCandidates = trainerIdCandidates;

    if (directCandidates.length) {
      const { data, error } = await supabase
        .from("trainer_pricing")
        .select("trainer_id")
        .in("trainer_id", directCandidates)
        .limit(1);
      ...
      if (
        ...
        return data[0].trainer_id as string;
      }
    }

    const possibleEmail =
      trainer?.email?.trim() ||
      (trainer?.full_name?.includes("@") ? trainer.full_name.trim() : null);
    ...
    if (profileError) {
      console.error("Failed to resolve profile by email:", profileError);
      return null;
    }

    return profileRow?.id ?? null;
  }, [trainer, trainerIdCandidates]);
  ...
  const loadPricing = useCallback(async () => {
    ...
    try {
      ...
      console.log("trainerIdCandidates:", trainerIdCandidates);

      const resolvedId = await resolvePricingTrainerId();

      console.log("resolved pricing trainer id:", resolvedId);

      if (!resolvedId) {
        setPricing(null);
        setPricingMessage(
          "Δεν βρέθηκε αντιστοίχιση προφίλ για την τιμολόγηση του προπονητή."
        );
        return;
      }
      ...
      console.log("trainer_pricing result:", data, error);
      ...
      setPricing(row as TrainerPricingRow);
      setPricingMessage("");
    } catch (e: any) {
  ...
  useEffect(() => {
    ...
    console.log("QuickBookModal trainer:", trainer);
    console.log("Resolved trainerId:", trainerId);
    console.log("Resolved trainerIdCandidates:", trainerIdCandidates);
...
const rawBookingPrice = getBookingPriceFromPricing(pricing, selectedSlot);
const finalBookingPrice = Number(rawBookingPrice ?? 0);

console.log("rawBookingPrice:", rawBookingPrice);
console.log("finalBookingPrice before insert:", finalBookingPrice);
...
const safeAmount = Number.isFinite(finalBookingPrice) ? finalBookingPrice : 0;
...
console.log("trainer_bookings insert payload:", insertPayload);
...
const { data: booking, error } = await supabase
  ...
  .single();
      ...
      const bookingWithPrice = {
        ...booking,
        amount: booking.amount ?? finalBookingPrice,
        currency_code: booking.currency_code ?? currencyCode,
        is_online: !!selectedSlot.is_online,
      };

      setSavedBookingPrice(booking.amount ?? finalBookingPrice);
...
function NoteStep({
  ...
  const disableSubmit = submitting || priceLoading;
  ...
  {priceLoading
    ...
    : "Θα οριστεί από τον προπονητή"}
      ...
      {!priceLoading && !pricingExists && (
        <div className="mb-4 flex items-center gap-2 rounded-2xl border border-amber-400/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
          <AlertTriangle className="h-4 w-4" />
          {pricingMessage || "Ο προπονητής δεν έχει ορίσει ακόμη τιμή. Η κράτηση μπορεί να σταλεί κανονικά."}
        </div>
      )}
