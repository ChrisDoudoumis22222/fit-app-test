...
const extractTrainerBookingPrice = (source: any): number | null => {
...
type TrainerLike = {
  ...
  full_name?: string;
  avatar_url?: string;
  booking_price?: number | string | null;
  price_per_booking?: number | string | null;
  session_price?: number | string | null;
  price?: number | string | null;
  hourly_rate?: number | string | null;
  per_session_price?: number | string | null;
} | null;
...
export default function QuickBookModal({
  ...
  const [trainerBookingPrice, setTrainerBookingPrice] = useState<number | null>(
    () => extractTrainerBookingPrice(trainer)
  );
  ...
  useEffect(() => {
    setTrainerBookingPrice(extractTrainerBookingPrice(trainer));
  }, [trainer]);

  useEffect(() => {
    const loadTrainerPrice = async () => {
      if (!open || !trainerId) return;

      const alreadyFound = extractTrainerBookingPrice(trainer);
      if (alreadyFound != null) {
        setTrainerBookingPrice(alreadyFound);
        return;
      }
      ...
      try {
        ...
        setTrainerBookingPrice(extractTrainerBookingPrice(data));
      } catch (e) {
        console.error("Failed to load trainer booking price:", e);
        setTrainerBookingPrice(null);
      } finally {
  ...
  const submitBooking = async () => {
    ...
    if (priceLoading) {
      setBanner({
        type: "error",
        title: "Περίμενε λίγο",
        message: "Γίνεται ακόμη φόρτωση της τιμής του προπονητή.",
      });
      return;
    }

    if (trainerBookingPrice == null || !Number.isFinite(trainerBookingPrice)) {
      setBanner({
        type: "error",
        title: "Δεν βρέθηκε τιμή",
        message:
          "Ο προπονητής δεν έχει ορίσει τιμή κράτησης ακόμη ή το πεδίο τιμής δεν βρέθηκε.",
      });
      return;
    }
    ...
    try {
      ...
      setSavedBookingPrice(trainerBookingPrice);
          ...
          >
            ...
            {step === "note" && selectedSlot && (
              <NoteStep
                ...
                bookingPrice={trainerBookingPrice}
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span>
            ...
            <strong className="font-semibold">
              {priceLoading ? "Φόρτωση..." : fmtPrice(bookingPrice)}
            </strong>
          </span>
        </div>
      </div>
  ...
  return (
    <div className="py-10 text-center">
      ...
      <p className="mb-6 text-sm text-white/80">
        Τιμή κράτησης:{" "}
        <strong className="font-semibold">{fmtPrice(bookingPrice)}</strong>
      </p>
