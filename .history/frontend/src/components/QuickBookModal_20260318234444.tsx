...
const PRICING_SELECT =
  "id, trainer_id, base_price, online_discount, package_discounts, specialty_pricing, updated_at, currency_code, payment_methods";
...
function resolveTrainerId(trainer: TrainerLite | null | undefined) {
  if (!trainer) return null;

  return (
    trainer.profile_id?.trim() ||
    trainer.trainer_id?.trim() ||
    trainer.id?.trim() ||
    trainer.user_id?.trim() ||
    null
  );
}

function resolveTrainerIdCandidates(trainer: TrainerLite | null | undefined) {
  if (!trainer) return [];
...
function resolvePossibleEmail(trainer: TrainerLite | null | undefined) {
  if (!trainer) return null;

  if (trainer.email?.trim()) return trainer.email.trim();
  if (trainer.full_name?.includes("@")) return trainer.full_name.trim();

  return null;
}

function pickPaymentMethod(
  paymentMethods: Record<string, any> | null | undefined
):
  | "cash"
  | "card"
  | "bank_transfer"
  ...
  if (pm.cash === true) return "cash";
  if (pm.card === true) return "card";
  if (pm.bank_transfer === true || pm.bankTransfer === true)
    return "bank_transfer";
  if (pm.wallet === true) return "wallet";
  if (pm.paypal === true) return "paypal";
...
type TrainerPricingRow = {
  id: string;
  trainer_id: string;
  base_price: number | string;
  online_discount: number | string;
  package_discounts: Record<string, any> | null;
  specialty_pricing: Record<string, any> | null;
  updated_at: string;
...
function getBookingPriceFromPricing(
  pricing: TrainerPricingRow | null,
  slot: SelectedSlot | null
) {
  if (!pricing || !slot) return null;

  const basePrice = asMoney(pricing.base_price, 0);
  const onlineDiscount = asMoney(pricing.online_discount, 0);

  let finalPrice = basePrice;

  if (slot.is_online) {
    finalPrice = basePrice - onlineDiscount;
  }
...
export default function QuickBookModal({
  ...
  const trainerId = useMemo(() => resolveTrainerId(trainer), [trainer]);
  const trainerIdCandidates = useMemo(
    () => resolveTrainerIdCandidates(trainer),
    [trainer]
  );
  ...
  const [pricing, setPricing] = useState<TrainerPricingRow | null>(null);
  const [pricingMessage, setPricingMessage] = useState<string>("");
  const [savedBookingPrice, setSavedBookingPrice] = useState<number | null>(
    null
  );
  ...
  const liveBookingPrice = useMemo(
    () => getBookingPriceFromPricing(pricing, selectedSlot),
    [pricing, selectedSlot]
  );

  const currencyCode = pricing?.currency_code || "EUR";
  const paymentMethod = useMemo(
    () => pickPaymentMethod(pricing?.payment_methods),
    [pricing]
  );
  ...
  const loadPricing = useCallback(async () => {
    ...
    try {
      ...
      let row: TrainerPricingRow | null = null;
      ...
      if (trainerId) {
        ...
        row = (data as TrainerPricingRow | null) ?? null;
      }

      if (!row && trainerIdCandidates.length) {
        const { data, error } = await supabase
          .from("trainer_pricing")
          .select(PRICING_SELECT)
          .in("trainer_id", trainerIdCandidates)
        ...
        row =
          Array.isArray(data) && data.length > 0
            ? (data[0] as TrainerPricingRow)
            : null;
      }

      if (!row) {
        const possibleEmail = resolvePossibleEmail(trainer);
      ...
      if (!row) {
        setPricing(null);
        setPricingMessage(
          "Δεν βρέθηκε τιμολόγηση για τον συγκεκριμένο προπονητή."
        );
        return;
      }
      ...
      console.error("Failed to load trainer pricing:", e);
      setPricing(null);
      setPricingMessage(
        e?.message || "Δεν ήταν δυνατή η φόρτωση της τιμολόγησης."
      );
    } finally {
      setPricingLoading(false);
    }
  }, [open, trainer, trainerId, trainerIdCandidates]);
  ...
  useEffect(() => {
    ...
    if (!trainerId) {
      setBanner({
        type: "error",
        title: "Μη έγκυρος προπονητής",
        message: "Δεν βρέθηκε σωστό trainer id για την κράτηση.",
      });
      return;
    }
    ...
    loadAvailability();
    loadPricing();
  }, [open, trainerId, loadAvailability, loadPricing]);
  ...
  const submitBooking = async () => {
    ...
    if (pricingLoading) {
      setBanner({
        type: "error",
        title: "Περίμενε λίγο",
        message: "Γίνεται ακόμα φόρτωση της τιμής του προπονητή.",
      });
      return;
    }

    if (!pricing || liveBookingPrice == null) {
      setBanner({
        type: "error",
        title: "Δεν υπάρχει τιμή",
        message:
          "Ο προπονητής δεν έχει ορίσει ακόμη τιμή, οπότε η κράτηση δεν μπορεί να ολοκληρωθεί.",
      });
      return;
    }

    const safeAmount = Number(
      Math.max(0, asMoney(liveBookingPrice, 0)).toFixed(2)
    );
    ...
    try {
      ...
      const { data: booking, error } = await supabase
        ...
        .select(
          "id, trainer_id, user_id, date, start_time, end_time, is_online, note, status, payment_status, payment_method, amount, currency_code, created_at"
        )
        .single();

      if (error) throw error;
      if (!booking?.id) throw new Error("Δεν επιστράφηκαν στοιχεία κράτησης.");

      const persistedAmount =
        booking?.amount != null && Number.isFinite(Number(booking.amount))
          ? Number(Number(booking.amount).toFixed(2))
          : safeAmount;

      const persistedCurrency =
        typeof booking?.currency_code === "string" && booking.currency_code
      ...
      const bookingWithPrice = {
        ...
        currency_code: persistedCurrency,
        is_online: !!selectedSlot.is_online,
      };

      setSavedBookingPrice(persistedAmount);
...
function NoteStep({
  ...
  const disableSubmit =
    submitting || priceLoading || !pricingExists || bookingPrice == null;
  ...
  return (
    <div>
          ...
          <span>
            ...
            <strong className="font-semibold">
              {priceLoading
                ? "Φόρτωση..."
                : pricingExists
                ? fmtPrice(bookingPrice, currencyCode)
                : "Δεν έχει οριστεί"}
            </strong>
          </span>
        </div>
      ...
      {!priceLoading && !pricingExists && (
        <div className="mb-4 flex items-center gap-2 rounded-2xl border border-red-400/30 bg-red-500/10 px-4 py-3 text-sm text-red-100">
          <AlertTriangle className="h-4 w-4" />
          {pricingMessage ||
            "Ο προπονητής δεν έχει ορίσει τιμή ακόμη. Η κράτηση δεν μπορεί να ολοκληρωθεί."}
        </div>
      )}
