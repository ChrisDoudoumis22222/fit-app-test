...
export default function ProfileSideCard({
  ...
  const toggleLike = useCallback(async () => {
    ...
    // optional guard (avoid liking yourself)
    ...
    try {
      ...
      if (already) {
        ...
        if (!error) {
          ...
          // if duplicates / race conditions happen, just treat as liked
          setLiked(true)
          pushToast("Ο επαγγελματίας αποθηκεύτηκε στα αγαπημένα")
        }
      }
...
<PremiumCard
  hover
  className={[
    // ✅ MOBILE: break out of parent container + stretch
    "block h-auto min-h-0 !h-auto !min-h-0",
    "relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] w-screen max-w-none",

    // ✅ SM+: back to normal card sizing
    "sm:static sm:left-auto sm:right-auto sm:ml-0 sm:mr-0 sm:w-full sm:max-w-[330px]",
    "lg:mx-0 lg:max-w-[340px] lg:self-start",
  ].join(" ")}
>
        <div className="p-3.5 sm:p-4">
          <div className="relative overflow-hidden rounded-2xl border border-zinc-700/50 bg-zinc-900/30">
            ...
            <div className="absolute left-3 top-3 flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-black/60 px-2.5 py-1.5 text-[11px] font-medium text-white backdrop-blur-md">
                <span
                  className={`h-2 w-2 rounded-full ${
                    data?.is_online ? "bg-green-400" : "bg-zinc-400"
                  }`}
                />
            ...
            <motion.button
              ...
              className={`absolute right-3 top-3 grid h-9 w-9 place-items-center rounded-full border border-white/10 backdrop-blur-md transition-all ${
          ...
          <div className="mt-3 space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center gap-1.5 rounded-xl border border-zinc-700/60 bg-zinc-900/50 px-2.5 py-1.5 text-zinc-200">
              ...
              {cat ? (
                <span className="inline-flex items-center gap-1.5 rounded-xl border border-zinc-700/60 bg-zinc-900/50 px-2.5 py-1.5 text-zinc-200">
                  {CatIcon ? (
                    <span className="inline-flex h-5 w-5 items-center justify-center rounded-md bg-zinc-800 border border-zinc-700/60">
            ...
            priceInfo?.typicalDurationMin ? (
              <div className="flex flex-wrap gap-2">
                {typeof priceInfo?.typicalPrice === "number" && priceInfo.typicalPrice > 0 ? (
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-zinc-700/60 bg-zinc-900/40 px-2.5 py-1 text-xs text-zinc-200">
            ...
            {Array.isArray(data?.roles) && data.roles.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {data.roles.slice(0, 4).map((t, i) => (
                  <span
                    key={`role-${i}`}
                    className="inline-flex items-center gap-1 rounded-full border border-zinc-700/60 bg-zinc-900/40 px-2.5 py-1 text-xs text-zinc-300"
                  >
                ...
                {data.roles.length > 4 ? (
                  <span className="rounded-full border border-zinc-700/60 bg-zinc-900/40 px-2.5 py-1 text-xs text-zinc-400">
                    +{data.roles.length - 4}
                  </span>
                ) : null}
              </div>
            ...
            <div className="pt-1 space-y-2.5">
              <motion.button
                ...
                className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-zinc-600/50 bg-gradient-to-r from-zinc-700 to-zinc-800 px-3.5 py-3 text-sm font-semibold text-zinc-100 shadow-lg backdrop-blur-xl hover:from-zinc-600 hover:to-zinc-700"
              >
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <button
                  ...
                  className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-zinc-700/60 bg-zinc-900/40 px-3 py-2.5 text-sm font-medium text-zinc-200 shadow-lg hover:bg-zinc-800/50"
                >
