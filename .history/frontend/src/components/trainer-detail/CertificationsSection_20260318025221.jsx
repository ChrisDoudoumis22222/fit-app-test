  ...
  return (
    <PremiumCard
      className={[
        ...
        // kill transitions / hover animations
        "!transition-none !duration-0 hover:!translate-y-0 hover:!scale-100 hover:!shadow-none",
      ].join(" ")}
    >
          ...
          {certs.length > 0 && (
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {certs.map((cert, idx) => (
                <div
                  key={`${cert}-${idx}`}
                  className="rounded-2xl border border-white/8 bg-zinc-900/35 p-3 shadow-sm backdrop-blur-sm transition-none sm:p-4"
                >
