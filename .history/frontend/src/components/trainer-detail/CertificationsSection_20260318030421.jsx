  ...
  return (
    <PremiumCard
      className={[
        ...
        "!transition-none !duration-0 !ease-linear !transform-none hover:!transform-none hover:!translate-y-0 hover:!scale-100 hover:!shadow-none",
      ].join(" ")}
    >
      <div className="px-4 py-5 sm:p-6 lg:p-8 xl:p-9">
        ...
        <div className="space-y-3 sm:space-y-4">
          {hasDiploma && (
            <div className="rounded-2xl bg-zinc-900/35 p-3 shadow-sm backdrop-blur-sm !transition-none sm:p-4 lg:p-5">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                {isUrl(diplomaUrl) && (
                  <a
                    ...
                    className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-zinc-800/70 px-4 py-2.5 text-sm font-medium text-zinc-200 !transition-none hover:bg-zinc-800/70 hover:text-zinc-200 sm:w-auto"
                  >
                    Προβολή
          ...
          {certs.length > 0 && (
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {certs.map((cert, idx) => (
                <div
                  key={`${cert}-${idx}`}
                  className="rounded-2xl border border-white/8 bg-zinc-900/35 p-3 shadow-sm backdrop-blur-sm !transition-none hover:shadow-none sm:p-4"
                >
