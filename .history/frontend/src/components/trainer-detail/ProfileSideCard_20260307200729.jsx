...
export default function ProfileSideCard({
  ...
  const onlineBadge = data?.is_online
    ? {
      ...
      >
        <div className="p-3 sm:p-4 lg:p-4.5">
          <div className="relative overflow-hidden rounded-[24px] border border-zinc-700/50 bg-zinc-900/30 shadow-[0_10px_40px_rgba(0,0,0,0.28)]">
            ...
            {/* top badges */}
            <div className="absolute left-3 top-3 right-14 flex flex-wrap items-center gap-2">
              <span className="inline-flex min-h-[34px] items-center gap-1.5 rounded-full border border-white/10 bg-black/65 px-3 py-1.5 text-[11px] font-medium text-white shadow-lg backdrop-blur-md">
                <span className={`h-2 w-2 rounded-full ${onlineBadge.dot}`} />
                <onlineBadge.Icon className="h-3.5 w-3.5" />
                <span className="truncate">{onlineBadge.label}</span>
              </span>
            ...
            <div className="absolute inset-x-3 bottom-3">
              <div className="rounded-[22px] border border-white/10 bg-black/60 p-3.5 backdrop-blur-xl">
          ...
          <div className="mt-3.5 space-y-3.5 sm:mt-4 sm:space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex min-h-[40px] items-center gap-1.5 rounded-2xl border border-zinc-700/60 bg-zinc-900/50 px-3 py-2 text-zinc-200">
              ...
              {cat ? (
                <span className="inline-flex min-h-[40px] items-center gap-2 rounded-2xl border border-zinc-700/60 bg-zinc-900/50 px-3 py-2 text-zinc-200">
                  {CatIcon ? (
            ...
            priceInfo?.typicalDurationMin ? (
              <div className="flex flex-wrap gap-2">
                ...
                priceInfo.typicalPrice > 0 ? (
                  <span className="inline-flex min-h-[36px] items-center gap-1.5 rounded-full border border-zinc-700/60 bg-zinc-900/40 px-3 py-1.5 text-xs text-zinc-200">
            ...
            {Array.isArray(data?.roles) && data.roles.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {data.roles.slice(0, 4).map((t, i) => (
                  <span
                    key={`role-${i}`}
                    className="inline-flex min-h-[34px] items-center gap-1.5 rounded-full border border-zinc-700/60 bg-zinc-900/40 px-3 py-1.5 text-xs text-zinc-300"
                  >
                ...
                {data.roles.length > 4 ? (
                  <span className="inline-flex min-h-[34px] items-center rounded-full border border-zinc-700/60 bg-zinc-900/40 px-3 py-1.5 text-xs text-zinc-400">
                    +{data.roles.length - 4}
                  </span>
                ) : null}
              </div>
            ...
            <div className="pt-1.5 space-y-2.5">
              <motion.button
                ...
                className="inline-flex min-h-[48px] w-full items-center justify-center gap-2 rounded-[18px] border border-zinc-600/50 bg-gradient-to-r from-zinc-700 to-zinc-800 px-4 py-3 text-sm font-semibold text-zinc-100 shadow-lg backdrop-blur-xl hover:from-zinc-600 hover:to-zinc-700"
              >
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <button
                  ...
                  className="inline-flex min-h-[46px] w-full items-center justify-center gap-2 rounded-[18px] border border-zinc-700/60 bg-zinc-900/40 px-3 py-3 text-sm font-medium text-zinc-200 shadow-lg hover:bg-zinc-800/50"
                >
