  ...
  return (
    <span
      className={cn(
        ...
        compact ? "min-h-[30px] px-2.5 py-1 text-[11px]" : "min-h-[36px] px-3 py-1.5 text-xs"
      )}
    >
      <Tag className={cn("shrink-0 text-zinc-400", compact ? "h-3 w-3" : "h-3.5 w-3.5")} />
      <span className={cn("truncate", compact ? "max-w-[118px]" : "max-w-[180px] sm:max-w-none")}>
        {children}
      </span>
    </span>
  )
...
export default function ProfileSideCard({
  ...
  const mobileVisibleSpecialties = specialties.slice(0, 3)
  const mobileHiddenSpecialties = specialties.slice(3)
  ...
  return (
    <>
      ...
      >
        <div className="p-2.5 pb-[5.25rem] sm:p-4 sm:pb-4">
          ...
          <div className="mt-3 space-y-2.5 px-1 sm:mt-4 sm:space-y-4 sm:px-0">
              ...
              <>
                <div className="sm:hidden">
                  <div className="flex flex-wrap gap-1.5">
                    {mobileVisibleSpecialties.map((item, i) => (
                    ...
                    {mobileHiddenSpecialties.length > 0 ? (
                      <button
                        type="button"
                        onClick={() => setMobileSpecialtiesExpanded((prev) => !prev)}
                      ...
                      >
                        {mobileSpecialtiesExpanded ? (
                          ...
                          <>
                            <ChevronDown className="h-3.5 w-3.5" />+{mobileHiddenSpecialties.length}
                          </>
                        )}
                      </button>
                    ) : null}
                  ...
                  <AnimatePresence initial={false}>
                    {mobileSpecialtiesExpanded && mobileHiddenSpecialties.length > 0 ? (
                      <motion.div
                      ...
                      >
                        <div className="mt-1.5 flex flex-wrap gap-1.5">
                          {mobileHiddenSpecialties.map((item, i) => (
