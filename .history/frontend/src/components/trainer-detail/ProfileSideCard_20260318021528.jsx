...
function normalizeSpecialties(data, cat) {
  const genericValues = new Set(["trainer", "user", "admin", "member"])

  const fromRoles = Array.isArray(data?.roles)
    ? data.roles.filter((r) => !genericValues.has(String(r || "").toLowerCase()))
    : []

  const fromSpecialties = Array.isArray(data?.specialties) ? data.specialties : []
  const fromSpecialtyArray = Array.isArray(data?.specialty) ? data.specialty : []

  const fromSingleSpecialty =
        ...
        <div className="p-3 pb-24 sm:p-4 lg:p-4.5">
          <div className="relative overflow-hidden rounded-[24px] border border-zinc-700/50 bg-zinc-900/30 shadow-[0_10px_40px_rgba(0,0,0,0.28)]">
            ...
            <motion.button
              ...
              className={cn(
                "absolute right-3 top-3 z-[3] grid h-10 w-10 place-items-center rounded-full border border-white/10 backdrop-blur-md transition-all touch-manipulation",
                liked
          ...
          <div className="mt-4 space-y-3.5 px-1 sm:mt-4.5 sm:space-y-4 sm:px-0">
            ...
            {specialties.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {specialties.slice(0, 6).map((item, i) => (
                  <span
                    key={`specialty-${i}`}
                    className="inline-flex min-h-[36px] items-center gap-1.5 rounded-full border border-zinc-700/60 bg-zinc-900/40 px-3 py-1.5 text-xs text-zinc-200 shadow-sm"
                  >
                    {i === 0 && CatIcon ? (
                    ...
                    <span className="max-w-[160px] truncate sm:max-w-none">{item}</span>
                  </span>
                ))}

                {specialties.length > 6 ? (
                  <span className="inline-flex min-h-[36px] items-center rounded-full border border-zinc-700/60 bg-zinc-900/40 px-3 py-1.5 text-xs text-zinc-400 shadow-sm">
                    +{specialties.length - 6}
                  </span>
                ) : null}
              </div>
            ...
            <div className="pt-1.5 space-y-2.5">
              <button
                ...
                className="hidden min-h-[50px] w-full items-center justify-center gap-2 rounded-[18px] border border-zinc-600/50 bg-gradient-to-r from-zinc-700 to-zinc-800 px-4 py-3 text-sm font-semibold text-zinc-100 shadow-lg backdrop-blur-xl transition sm:inline-flex"
              >
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <button
                  ...
                  className="inline-flex min-h-[46px] w-full items-center justify-center gap-2 rounded-[18px] border border-zinc-700/60 bg-zinc-900/40 px-3 py-3 text-sm font-medium text-zinc-200 shadow-lg transition touch-manipulation"
                >
