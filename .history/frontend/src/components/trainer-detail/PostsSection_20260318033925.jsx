...
function MobileScrollSkeletonCard() {
  return (
    <div className="w-[84vw] max-w-[340px] shrink-0 snap-start overflow-hidden rounded-[24px] border border-white/10 bg-zinc-950/60">
...
function PostsSectionSkeleton() {
  return (
    ...
    >
      ...
      {/* mobile */}
      <div className="flex gap-4 overflow-x-auto pb-2 pr-4 md:hidden [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {Array.from({ length: 5 }).map((_, i) => (
          <MobileScrollSkeletonCard key={i} />
        ))}
      </div>
  ...
  return (
    <div
      className={cn(
        ...
        "opacity-100 md:opacity-0 md:translate-y-1 md:transition md:duration-300 md:group-hover:translate-y-0 md:group-hover:opacity-100"
      )}
    >
      {likes > 0 && (
...
function MobileScrollPostCard({ post, onClick, priority = false }) {
  ...
  return (
    <motion.button
      ...
      className="group w-[84vw] max-w-[340px] shrink-0 snap-start overflow-hidden rounded-[24px] border border-white/10 bg-zinc-950/40 text-left"
    >
      <div className="relative h-[240px]">
        <LazyPostImage
          src={postImage}
      ...
      <div className="p-4">
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          <span className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/15 bg-white/[0.06] text-white backdrop-blur">
    ...
    <>
      ...
      >
        ...
        <div className="md:hidden">
          <div className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-2 pr-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {posts.slice(0, 5).map((post, index) => (
              <MobileScrollPostCard
                key={post.id}
                post={post}
                priority={index === 0}
