...
function Button({
  ...
  const variants = {
    ...
    secondary:
      "bg-white/75[0.05] hover:bg-white/75[0.08] text-white border border-white/8",
    ghost:
      "bg-transparent hover:bg-white/75[0.05] text-white border border-white/8",
    calendar:
...
function InfoRow({ icon: Icon, label, value }) {
  return (
    <div className="flex items-start gap-3 rounded-2xl bg-white/75[0.035] px-3 py-3">
      <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white/75[0.04] border border-white/6">
...
function SummaryChip({ label, value }) {
  return (
    <div className="rounded-2xl bg-white/75[0.04] px-3 py-2">
        ...
        <div className="flex h-full w-full flex-col bg-[#090a0d]">
          <div className="sticky top-0 z-10 border-b border-white/6 bg-[#090a0d]/96 px-4 pb-3 pt-4 backdrop-blur">
            <div className="mb-3 flex justify-center">
              <div className="h-1.5 w-11 rounded-full bg-white/7510" />
            </div>
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              {onClose && (
                <button
                  onClick={onClose}
                  aria-label="Κλείσιμο"
                  className="grid h-10 w-10 place-items-center rounded-full border border-white/8 bg-white/75[0.04] text-white/70 transition hover:bg-white/7510 hover:text-white"
                >
                  <X className="h-5 w-5" />
                </button>
              )}
    ...
    <Glass accepted={status === "accepted"} className="h-fit p-4 sm:p-5">
      ...
      {!id ? null : loading ? (
        <div className="animate-pulse space-y-3">
          <div className="h-7 w-40 rounded border border-white/10 bg-white/75[.08]" />
          <div className="h-32 rounded-xl border border-white/10 bg-white/75[.06]" />
          <div className="h-6 rounded border border-white/10 bg-white/75[.06]" />
          <div className="h-6 rounded border border-white/10 bg-white/75[.06]" />
        </div>
      ) : err ? (
              ...
              <div
                className={`grid h-14 w-14 place-items-center rounded-full border shadow-[0_8px_24px_rgba(0,0,0,.35)] shrink-0 sm:h-16 sm:w-16 ${
                  status === "accepted"
                    ...
                    : "border-white/10 bg-white/758 ring-2 ring-white/10"
                }`}
              >
    ...
    <div className="space-y-4">
      {!id ? null : loading ? (
        <div className="space-y-3 animate-pulse">
          <div className="h-24 rounded-3xl bg-white/75[.05]" />
          <div className="h-16 rounded-2xl bg-white/75[.05]" />
          <div className="h-16 rounded-2xl bg-white/75[.05]" />
          <div className="h-16 rounded-2xl bg-white/75[.05]" />
        </div>
      ) : err ? (
        ...
        <>
          <div className="space-y-4">
                ...
                {clientAvatar ? (
                  ...
                  <div
                    className={`grid h-14 w-14 place-items-center rounded-full border ${
                      status === "accepted"
                        ? "border-emerald-300/20 bg-emerald-500/10 ring-2 ring-emerald-300/10"
                        : "border-white/10 bg-white/756 ring-2 ring-white/8"
                    }`}
                  >
            ...
            <div className="rounded-2xl bg-white/75[0.03]">
              <button
                type="button"
              ...
              >
                ...
                <div className="grid h-9 w-9 place-items-center rounded-full bg-white/75[0.04] border border-white/6">
                  {mobileExpanded ? (
