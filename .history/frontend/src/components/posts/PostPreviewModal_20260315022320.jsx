  ...
  const renderDefaultContent = ({ mobile = false }) => (
    <div
      className={
        mobile
          ? "space-y-5"
          : "grid grid-cols-1 xl:grid-cols-[1.15fr_.85fr] gap-6"
      }
    >
      {!!images.length && (
      ...
      <div className="min-w-0">
        <div
          className={
            mobile
              ? "px-0 pt-1 pb-2"
              : "rounded-[24px] border border-white/10 bg-white/[0.03] p-6 backdrop-blur-xl"
          }
        >
          <div className={mobile ? "space-y-4 px-0" : "space-y-4"}>
            <h1
              className={[
      ...
      <div className="absolute inset-0 hidden items-center justify-center p-5 sm:flex">
        ...
        >
          ...
          <div className="qb-scroll relative max-h-[92vh] overflow-y-auto p-6">
            {PostCard ? (
              <div className="rounded-[28px] border border-white/10 bg-black/20 p-3 backdrop-blur-xl">
      ...
      <div className="absolute inset-0 sm:hidden">
        ...
        >
          <div className="relative border-b border-zinc-800/80 bg-black/90 px-4 pb-4 pt-[max(env(safe-area-inset-top),16px)] backdrop-blur-xl">
            ...
            <div className="pr-14">
              <h2 className="text-base font-semibold text-white">Προεπισκόπηση</h2>
              <p className="mt-1 text-sm text-zinc-400">Δες το post σε mobile μορφή</p>
            </div>
          </div>

          <div className="qb-scroll flex-1 overflow-y-auto px-4 py-4">
            {PostCard ? (
