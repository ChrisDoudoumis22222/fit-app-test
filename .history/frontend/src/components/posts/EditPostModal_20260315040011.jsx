          ...
          <div className="relative overflow-hidden rounded-[28px] bg-white px-5 pb-5 pt-6 shadow-[0_24px_70px_rgba(0,0,0,0.35)] sm:px-6 sm:pb-6 sm:pt-7">
            ...
            <div className="relative mx-auto grid h-[118px] w-[118px] place-items-center">
              <div
                className={cn(
                  ...
                  isSuccess
                    ? "bg-gradient-to-br from-[#7C86FF] via-[#5B6CFF] to-[#5361E9]"
              ...
              <span className="absolute left-[16px] top-[24px] h-1.5 w-1.5 rounded-full bg-white/90" />
              <span className="absolute right-[18px] top-[22px] h-1.5 w-1.5 rounded-full bg-white/85" />
              <span className="absolute left-[20px] bottom-[24px] h-1.5 w-1.5 rounded-full bg-white/80" />
              <span className="absolute right-[16px] bottom-[26px] h-1.5 w-1.5 rounded-full bg-white/90" />
              <span className="absolute left-[28px] top-[50px] h-2 w-2 rotate-45 bg-white/80" />
              <span className="absolute right-[30px] top-[50px] h-2 w-2 rotate-45 bg-white/75" />
            ...
            <button
              ...
              className={cn(
                ...
                isSuccess
                  ? "bg-gradient-to-r from-[#5F6FFF] via-[#4E5FEF] to-[#6678FF] shadow-[0_16px_30px_rgba(95,111,255,0.35)] hover:brightness-105"
...
export default function EditPostModal({ open, post, onClose, onSave }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      closeThenShowResult({
        type: "success",
        title: "Verified",
        message: "Οι αλλαγές στην ανάρτησή σου αποθηκεύτηκαν με επιτυχία.",
        primaryLabel: "Τέλεια",
      })
    } catch {
      setResultState({
  ...
  if (!mounted || ((!shouldRender && !open) && !resultState) || !post) {
    if (!mounted || !resultState) return null
  }
