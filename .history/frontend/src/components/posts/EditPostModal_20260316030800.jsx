...
import SuccessStatusModal from "./edit/SuccessStatusModal"
...
const isAcceptedImageFile = (file) => {
  ...
  if (ACCEPTED_IMAGE_MIME_TYPES.includes(mime)) return true
...
const Field = ({
  ...
  <div
    className={cn(
      "space-y-2",
      error
        ? "rounded-2xl bg-red-500/[0.05] p-2 ring-1 ring-red-500/30"
  ...
  >
    ...
    {textarea ? (
      <textarea
        className={cn(
          "h-32 w-full resize-none rounded-2xl px-4 py-3 text-zinc-100 placeholder-zinc-500 transition-all duration-200 focus:outline-none",
          error
            ? "border border-red-500/60 bg-red-500/[0.04] focus:border-red-500"
            : "border border-zinc-800 bg-zinc-900 focus:border-zinc-600",
        )}
      ...
      <input
        className={cn(
          "w-full rounded-2xl px-4 py-3 text-zinc-100 placeholder-zinc-500 transition-all duration-200 focus:outline-none",
          error
            ? "border border-red-500/60 bg-red-500/[0.04] focus:border-red-500"
            : "border border-zinc-800 bg-zinc-900 focus:border-zinc-600",
        )}
...
const ErrorStatusCard = ({ result, onPrimary, onSecondary }) => {
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        >
          <div className="relative overflow-hidden rounded-[30px] bg-[radial-gradient(circle_at_top,rgba(66,16,16,0.38),transparent_38%),linear-gradient(180deg,rgba(12,13,18,0.98),rgba(8,9,12,0.98))] px-5 pb-5 pt-6 shadow-[0_24px_80px_rgba(0,0,0,0.55)] sm:px-6 sm:pb-6 sm:pt-7">
            <div className="pointer-events-none absolute left-1/2 top-5 h-36 w-36 -translate-x-1/2 rounded-full bg-rose-500 blur-3xl opacity-20" />

            <div className="relative flex flex-col items-center text-center">
              <div className="relative mb-5 mt-1 flex h-[116px] w-[116px] items-center justify-center sm:h-[124px] sm:w-[124px]">
                <div className="absolute inset-0 rounded-full border border-rose-400/20 bg-rose-500/8" />
                <div className="absolute inset-[18px] rounded-full border border-rose-400/25 bg-rose-500/8" />
                <div className="absolute inset-[36px] rounded-full border border-rose-300/30 bg-rose-500/10 backdrop-blur-sm" />
                <div className="relative grid h-10 w-10 place-items-center rounded-full border border-rose-300/40 bg-rose-400/10 text-rose-300 shadow-[0_8px_22px_rgba(0,0,0,0.28)] sm:h-11 sm:w-11">
                  <AlertTriangle className="h-5 w-5" />
                </div>
              </div>

              <div className="mb-5 inline-flex h-8 items-center rounded-full border border-rose-400/20 bg-rose-500/10 px-4 text-[12px] font-bold tracking-[0.18em] text-rose-300">
                ERROR
              </div>
...
export default function EditPostModal({ open, post, onClose, onSave }) {
  ...
  const [validationErrors, setValidationErrors] = useState(EMPTY_VALIDATION)
  ...
  const closeThenShowSuccess = useCallback(
    (result) => {
      suppressOpenSyncRef.current = true
      setIsVisible(false)
  ...
  const handleDescChange = useCallback((value) => {
    setEditDesc(value)
    setValidationErrors((prev) =>
      prev.description ? { ...prev, description: "" } : prev,
    )
  }, [])
    ...
    try {
      ...
      for (const file of incoming) {
        if (!isAcceptedImageFile(file)) {
          rejectedType.push(file.name || "file")
          continue
        }
      ...
      for (const file of acceptedFiles) {
        try {
          ...
          if (finalFile.size > MAX_BYTES) {
            rejectedSize.push(file.name || "file")
            continue
          }
      ...
      if (rejectedType.length) {
        feedbackParts.push(
          "Επιτρέπονται μόνο PNG, JPG, JPEG και WEBP αρχεία.",
        )
      }

      if (rejectedSize.length) {
        feedbackParts.push("Κάθε εικόνα πρέπει να είναι έως 2MB μετά την επεξεργασία.")
      }

      setFileFeedback(feedbackParts.join(" "))
    } finally {
      setProcessingImages(false)
    }
  }, [])
  ...
  const handleSave = async () => {
    ...
    const nextErrors = {
      title: editTitle.trim() ? "" : "Ο τίτλος είναι υποχρεωτικός.",
      description: editDesc.trim() ? "" : "Η περιγραφή είναι υποχρεωτική.",
      images: totalImages > 0 ? "" : "Πρόσθεσε τουλάχιστον μία εικόνα.",
    }

    setValidationErrors(nextErrors)

    if (nextErrors.title || nextErrors.description || nextErrors.images) {
      return
    }
  ...
  if (!shouldRender && !errorState && !successState) return null
  if (!post && !errorState && !successState && !shouldRender) return null
    ...
    >
      <div className="space-y-6">
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <label
                className={cn(
                  "block text-sm font-medium",
                  validationErrors.images ? "text-red-400" : "text-zinc-300",
                )}
              >
                Εικόνες ({totalImages})
              </label>
          ...
          {(validationErrors.images || fileFeedback) && (
            <div className="space-y-1">
              ...
              {!!fileFeedback && (
                <p className="text-sm font-medium text-red-400">{fileFeedback}</p>
              )}
            </div>
          )}
          ...
          {!totalImages && (
            <div
              className={cn(
                ...
                validationErrors.images
                  ? "border border-dashed border-red-500/50 bg-red-500/[0.04] text-red-400"
                  : "border border-dashed border-zinc-800 bg-zinc-950 text-zinc-500",
              )}
            >
              <ImageIcon
                className={cn(
                  "mx-auto mb-2 h-10 w-10",
                  validationErrors.images ? "text-red-400" : "text-zinc-600",
                )}
              />
  ...
  const modalNode = (
    <>
      ...
      {!!errorState && !shouldRender && (
        <ErrorStatusCard
          result={errorState}
          onPrimary={() => setErrorState(null)}
          onSecondary={() => setErrorState(null)}
        />
      )}

      <SuccessStatusModal
        open={!!successState && !shouldRender}
        title={successState?.title}
        message={successState?.message}
        primaryLabel={successState?.primaryLabel || "Τέλεια"}
