...
const VALIDATION_HIGHLIGHT_MS = 3 * 60 * 1000 // 3 minutes
...
const Field = ({
  ...
  <div
    className={cn(
      ...
      invalid
        ? "p-2 ring-1 ring-red-500/40 bg-red-500/[0.04] shadow-[0_0_0_1px_rgba(239,68,68,0.10)] animate-[qbValidationPulse_2.2s_ease-in-out_infinite]"
        : dirty
  ...
  >
    <label
      className={cn(
        ...
        invalid ? "text-red-300" : "text-zinc-300",
      )}
    >
      {label}
    </label>
    ...
    {textarea ? (
      <textarea
        className={cn(
          "h-32 w-full resize-none rounded-2xl border bg-zinc-900 px-4 py-3 text-zinc-100 placeholder-zinc-500 transition-all duration-200 focus:outline-none",
          invalid
      ...
      <input
        className={cn(
          "w-full rounded-2xl border bg-zinc-900 px-4 py-3 text-zinc-100 placeholder-zinc-500 transition-all duration-200 focus:outline-none",
          invalid
            ? "border-red-500/50 focus:border-red-400"
            : "border-zinc-800 focus:border-zinc-600",
        )}
...
export default function EditPostModal({ open, post, onClose, onSave }) {
  ...
  const clearInvalidFields = useCallback((keys = []) => {
  ...
  const triggerValidationHighlight = useCallback(
    ({ title = false, desc = false, images = false }) => {
      setInvalidFields({ title, desc, images })
      ...
      if (title || desc || images) {
        validationTimerRef.current = setTimeout(() => {
          setInvalidFields({ title: false, desc: false, images: false })
  ...
  const handleTitleChange = (value) => {
    setEditTitle(value)
    if (value.trim()) clearInvalidFields(["title"])
  }

  const handleDescChange = (value) => {
    setEditDesc(value)
    if (value.trim()) clearInvalidFields(["desc"])
  }

  const handleSave = async () => {
    const missingTitle = !editTitle.trim()
    const missingDesc = !editDesc.trim()
    const totalImages = editImages.length + newThumbs.length
    const missingImages = totalImages === 0

    if (missingTitle || missingDesc || missingImages) {
  ...
  useEffect(() => {
    if (editImages.length + newThumbs.length > 0) {
      clearInvalidFields(["images"])
    }
  }, [editImages.length, newThumbs.length, clearInvalidFields])
      ...
      <div className="space-y-6">
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <label
                className={cn(
                  "block text-sm font-medium",
                  invalidFields.images ? "text-red-300" : "text-zinc-300",
                )}
              >
                Εικόνες ({totalImages})
              </label>
            ...
            <button
              ...
              className={cn(
                "inline-flex shrink-0 items-center gap-2 rounded-2xl border bg-zinc-900 px-3 py-2 text-sm text-white transition disabled:opacity-50",
                invalidFields.images
                  ? "border-red-500/40 hover:bg-zinc-800"
          ...
          {!totalImages && (
            <div
              className={cn(
                "rounded-2xl border py-10 text-center text-zinc-500 transition-all duration-300",
                invalidFields.images
            ...
            >
              <ImageIcon
                className={cn(
                  ...
                  invalidFields.images ? "text-red-400/70" : "text-zinc-600",
                )}
              />
              <p className={invalidFields.images ? "text-red-300" : ""}>
                Δεν υπάρχουν εικόνες. Πρόσθεσε τουλάχιστον μία.
              </p>
            </div>
          )}
