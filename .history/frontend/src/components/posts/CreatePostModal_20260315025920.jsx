  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[130] flex items-end justify-center p-4 sm:items-center sm:p-6"
      >
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        <motion.div
          ...
          className={cn(
            "relative w-full max-w-[380px] overflow-hidden rounded-[30px] border shadow-2xl",
            "bg-gradient-to-b from-zinc-950/98 via-black to-black",
            "border-white/10",
          )}
        ...
        >
          <div
            className={cn(
              "pointer-events-none absolute inset-x-0 top-0 h-24 opacity-80 blur-3xl",
          ...
          <div
            className={cn(
              "pointer-events-none absolute left-1/2 top-[-42px] h-36 w-36 -translate-x-1/2 rounded-full blur-3xl",
              isSuccess ? "bg-emerald-400/14" : "bg-red-400/14",
            )}
          />
          ...
          <div className="relative px-5 pb-5 pt-6 sm:px-6 sm:pb-6 sm:pt-7">
            <div className="flex justify-center">
              <div
                className={cn(
                  ...
                  isSuccess
                    ? "border-emerald-400/15 bg-[radial-gradient(circle_at_30%_30%,rgba(16,185,129,0.30),rgba(16,185,129,0.09)_48%,transparent_70%)]"
                    : "border-red-400/15 bg-[radial-gradient(circle_at_30%_30%,rgba(239,68,68,0.30),rgba(239,68,68,0.08)_48%,transparent_70%)]",
                )}
              >
                <div
                  className={cn(
                    "grid h-14 w-14 place-items-center rounded-full border backdrop-blur-xl",
                    isSuccess
                      ? "border-emerald-300/20 bg-emerald-400/12 text-emerald-300"
                      : "border-red-300/20 bg-red-400/12 text-red-300",
                  )}
                >
                  {isSuccess ? (
            ...
            <div className="mt-5 text-center">
              ...
              <p className="mx-auto mt-2 max-w-[280px] text-sm leading-6 text-zinc-300">
                {result.message}
              </p>
            </div>
            ...
            <div className="mt-6 space-y-2">
              <button
                ...
                onClick={onPrimary}
                className={cn(
                  "w-full h-12 rounded-2xl font-semibold transition-all duration-200",
              ...
              {!isSuccess && (
                <button
                  type="button"
                  onClick={onSecondary}
                  className="w-full h-11 rounded-2xl border border-white/10 bg-white/[0.04] text-white/88 transition hover:bg-white/[0.08]"
                >
                  Κλείσιμο
                </button>
              )}
...
export default function CreatePostModal({
  ...
  const mobileCloseY = useMemo(() => {
    ...
    return Math.max(window.innerHeight || MOBILE_CLOSE_Y_FALLBACK, MOBILE_CLOSE_Y_FALLBACK)
  }, [shouldRender])
  ...
  const handleCreate = useCallback(async () => {
    if (!uid) {
      ...
      setResultState({
        ...
        message: "Δεν βρέθηκε ο λογαριασμός σου. Κάνε ανανέωση και προσπάθησε ξανά.",
      })
      return
    }
  ...
  }, [uid, title, desc, files, onCreated, onError, onSuccess])
    ...
    <>
      ...
      >
        <motion.button
          ...
          onClick={resultState ? undefined : requestClose}
          initial={false}
