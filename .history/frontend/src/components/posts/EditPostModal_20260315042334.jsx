...
const ResultStatusCard = ({ result, onPrimary, onSecondary, mobile }) => {
  if (!result) return null

  const tone = result.type === "decline" ? "decline" : "error"
  const isDecline = tone === "decline"
  const badgeLabel = isDecline ? "DECLINED" : "ERROR"
    ...
    return (
      <AnimatePresence>
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[140] flex items-end justify-center p-3 sm:hidden"
        >
          <motion.button
            type="button"
          ...
          >
            <div className="relative overflow-hidden rounded-[30px] bg-[radial-gradient(circle_at_top,rgba(66,16,16,0.38),transparent_38%),linear-gradient(180deg,rgba(12,13,18,0.98),rgba(8,9,12,0.98))] px-5 pb-5 pt-6 shadow-[0_24px_80px_rgba(0,0,0,0.55)]">
              <div
                className={cn(
                  "pointer-events-none absolute left-1/2 top-5 h-36 w-36 -translate-x-1/2 rounded-full blur-3xl opacity-20",
                  isDecline ? "bg-amber-400" : "bg-rose-500",
                )}
              />

              <div className="relative flex flex-col items-center text-center">
                <div className="relative mb-5 mt-1 flex h-[116px] w-[116px] items-center justify-center">
                  <div
                    className={cn(
                  ...
                  <div
                    className={cn(
                      "relative grid h-10 w-10 place-items-center rounded-full border shadow-[0_8px_22px_rgba(0,0,0,0.28)]",
                      isDecline
                ...
                <p className="mt-3 max-w-[300px] text-[15px] leading-7 text-zinc-300">
                  {result.message}
                </p>
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[140] hidden items-end justify-center p-4 sm:flex sm:items-center sm:p-6"
      >
        <motion.button
          type="button"
          aria-label="close result"
        ...
        <motion.div
          ...
          className="relative w-full max-w-[390px]"
        >
          <div className="relative overflow-hidden rounded-[28px] bg-[linear-gradient(180deg,rgba(19,19,24,0.98),rgba(10,10,14,0.98))] px-5 pb-5 pt-6 shadow-[0_24px_70px_rgba(0,0,0,0.45)] sm:px-6 sm:pb-6 sm:pt-7">
            <div
              className={cn(
                "pointer-events-none absolute left-1/2 top-7 h-24 w-24 -translate-x-1/2 rounded-full blur-2xl opacity-25",
                isDecline ? "bg-amber-500" : "bg-rose-500",
              )}
            />
              ...
              <div
                className={cn(
                  ...
                  isDecline
                    ...
                    : "bg-gradient-to-br from-[#FF8A8A] via-[#FF5F7A] to-[#FF6B3D]",
                )}
              />
              <div className="absolute inset-[15px] rounded-full border border-white/20 bg-white/10" />
              <div className="relative grid h-14 w-14 place-items-center rounded-full bg-white shadow-[0_12px_30px_rgba(255,255,255,0.18)]">
                <AlertTriangle
                  className={cn(
                    "h-8 w-8",
                    isDecline ? "text-amber-500" : "text-rose-500",
                  )}
                />
              </div>
            </div>
            ...
            <div className="mt-4 text-center">
              <h3 className="text-[24px] font-black tracking-tight text-white sm:text-[26px]">
                {result.title}
              </h3>

              <p className="mx-auto mt-2 max-w-[290px] text-[14px] leading-6 text-zinc-300 sm:text-[15px]">
                {result.message}
              </p>
            </div>
            ...
            <button
              ...
              onClick={onPrimary}
              className={cn(
                "mt-6 h-12 w-full rounded-full px-4 text-sm font-semibold text-white transition active:scale-[0.99]",
                isDecline
                  ? "bg-gradient-to-r from-[#F59E0B] via-[#D97706] to-[#B45309] shadow-[0_16px_30px_rgba(217,119,6,0.28)] hover:brightness-105"
                  : "bg-gradient-to-r from-[#FF7D7D] via-[#FF5E73] to-[#FF6B4A] shadow-[0_16px_30px_rgba(255,94,115,0.30)] hover:brightness-105",
              )}
            >
              {result.primaryLabel || "Εντάξει"}
            </button>
...
export default function EditPostModal({ open, post, onClose, onSave }) {
  ...
  const closeThenCloseOnly = useCallback(() => {
    suppressOpenSyncRef.current = true
    setIsVisible(false)
