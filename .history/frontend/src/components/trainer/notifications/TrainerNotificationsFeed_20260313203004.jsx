...
function mapFavoriteTrainerPostNotification(p) {
  const trainerName = getDisplayName(p) || "Χρήστης";
  const previewText = truncateText(
    p?.caption || p?.content || p?.description || p?.title || ""
  );
  ...
  return {
    ...
    headline: `❤️ ${trainerName} δημοσίευσε νέα ανάρτηση`,
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="min-w-0 flex-1">
              <p className="line-clamp-2 text-sm font-semibold text-zinc-200">
                {item.headline || `❤️ ${item.trainer_name || "Χρήστης"} δημοσίευσε νέα ανάρτηση`}
              </p>
