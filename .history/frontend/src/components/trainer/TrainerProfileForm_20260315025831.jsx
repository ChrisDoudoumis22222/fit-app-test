...
className={[
  ...
  // MOBILE — completely flat, no background
  "-mx-3 px-3 rounded-none border-0 bg-transparent shadow-none backdrop-blur-0",
  ...
  gradient ? "premium-gradient sm:premium-gradient" : "",
...
function TrainerProfileForm({ profile, onAfterSave, pvDebug = false }) {
  ...
  const dirtyMap = useMemo(() => {
    ...
    return {
      ...
      specialty: form.specialty !== saved.specialty,
      ...
      selectedSpecialties: !areArraysEqual(
        form.selectedSpecialties,
        saved.selectedSpecialties
      ),
    };
  }, [form]);
  ...
  const specialtiesDirty = dirtyMap.selectedSpecialties;
...
<p className="text-xs sm:text-sm text-zinc-400">
  Συμπλήρωσε τα προσωπικά και επαγγελματικά σου στοιχεία
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <FieldChip show={specialtiesDirty} text="Ειδικότητες" />
          </div>
        </div>
      </motion.header>
        ...
        <div className="space-y-6 sm:space-y-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 sm:gap-6 items-stretch">
            ...
            <PremiumCard
              ...
              subtitle="Κατηγορία, πόλη και εμπειρία"
              icon={<BriefcaseBusiness className="h-6 w-6" />}
              dirty={professionalDirty}
            >
              <div className="space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div
                    className={cn(
                      "space-y-2",
                      dirtyMap.specialty ? "halo p-2 -m-2 rounded-2xl" : ""
                    )}
                  >
                    <label className="text-sm font-medium text-white flex items-center gap-2">
                ...
                <div className="space-y-12">
                  ...
                  <div
                    className={cn(
                      "mt-16 sm:mt-20 h-full rounded-2xl border p-5 sm:p-6 sm:bg-transparent sm:border-0 sm:p-0",
                      dirtyMap.isOnline
                        ? "halo border-white/10 bg-gradient-to-br from-zinc-900/80 to-black/70"
                        : "border-white/10 bg-zinc-900/40"
                    )}
                  >
                    <div className="flex h-full items-start gap-4">
                      <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-gradient-to-br from-white/14 to-white/5 shadow-[inset_0_1px_0_rgba(255,255,255,0.10),0_8px_20px_rgba(0,0,0,0.22)] sm:h-14 sm:w-14">
                        {form.isOnline ? (
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-3 text-[13px] leading-6 text-zinc-400 sm:text-sm sm:leading-7">
                          Θα εμφανίζεσαι ως διαθέσιμος για online συνεδρίες
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          {form.specialty && currentCategory && (
            <PremiumCard
              title="Ειδικότητες"
              subtitle={`Διάλεξε όσες σε περιγράφουν καλύτερα · ${currentCategory.label}`}
                ...
                {form.selectedSpecialties.length > 0 && (
                  <div className="mt-2">
                    <p className="text-sm sm:text-base text-white/60 mb-4 font-medium">
                      Επιλεγμένες ({form.selectedSpecialties.length}):
                    </p>
