...
export default function DiplomaCard({
  ...
  const uploadDiploma = async (e) => {
    ...
    try {
      ...
      const processed = await prepareCompressedUploadFile(rawFile, {
        maxInputBytes: MAX_FILE_BYTES,
        hardMaxBytes: MAX_FILE_BYTES,
      ...
      const safeExt =
        ext || (processed.fileKind === "pdf" ? "pdf" : "jpg");

      const filePath = `${user.id}/diploma-${Date.now()}.${safeExt}`;

      const { error: upErr } = await supabase.storage.from(bucket).upload(filePath, uploadFile, {
        cacheControl: "3600",
        upsert: true,
        contentType: uploadFile.type || processed.type || undefined,
      });
      ...
      if (processed.fileKind === "pdf") {
        flash("success", `Το PDF ανέβηκε (${fmtBytes(processed.outputBytes)}).`);
      } else if (processed.skipped) {
        flash("success", `Η εικόνα ανέβηκε (${fmtBytes(processed.outputBytes)}).`);
      } else {
        flash(
          "success",
          `Η εικόνα βελτιστοποιήθηκε και ανέβηκε: ${fmtBytes(processed.originalBytes)} → ${fmtBytes(processed.outputBytes)}.`
        );
      }
  ...
  return (
    <div className={["w-full", className].join(" ")}>
      ...
      >
        <div className="pointer-events-none absolute -top-24 -right-24 hidden h-64 w-64 rounded-full bg-white/[0.04] blur-3xl sm:block" />
        <div className="pointer-events-none absolute -bottom-28 -left-28 hidden h-72 w-72 rounded-full bg-white/[0.03] blur-3xl sm:block" />
          ...
          <div className="mx-auto flex max-w-xl flex-col items-center text-center">
            <div className="flex w-full justify-center">
              {hasDiploma ? (
                previewIsImage ? (
                  <div className="relative flex w-full justify-center">
                    <img
                      ...
                      loading="lazy"
                      decoding="async"
                      className="h-auto max-h-40 w-auto max-w-full rounded-2xl bg-transparent object-contain shadow-none sm:max-h-64 sm:rounded-3xl sm:bg-white/5 sm:shadow-[0_18px_55px_rgba(0,0,0,.45)]"
                    />
                    <div className="pointer-events-none absolute -inset-4 -z-10 hidden rounded-[32px] bg-white/5 blur-xl sm:block" />
                  </div>
                ) : (
                  <div className="w-full max-w-md rounded-2xl border-0 bg-transparent p-4 text-left shadow-none sm:rounded-3xl sm:border sm:border-white/10 sm:bg-white/5 sm:p-5 sm:shadow-[0_18px_55px_rgba(0,0,0,.35)]">
                ...
                <div className="w-full max-w-md rounded-2xl border-0 bg-transparent p-6 sm:rounded-3xl sm:border sm:border-dashed sm:border-white/10 sm:bg-white/[0.04] sm:p-8">
                  ...
                  <div className="mt-4 text-sm font-medium text-white/80 sm:text-base">
            ...
            <div className="mt-5 text-sm font-medium text-white/80 sm:text-base">
              {hasDiploma
            ...
            <div className="mt-2 text-xs text-white/45 sm:text-sm">
              PDF / PNG / JPG / JPEG / WEBP • μέγιστο αρχείο {fmtBytes(MAX_FILE_BYTES)}
            </div>
            ...
            {hasDiploma ? (
              <div className="mt-4">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-2 text-sm text-white/85 hover:bg-white/10"
                >
            ...
            <div className="mt-7 w-full">
              <div className="mx-auto flex w-full max-w-sm flex-col gap-2 sm:flex-row sm:gap-3">
                <button
                  type="button"
            ...
            {(notice || error) && (
              <div className="mt-6 flex w-full justify-center">
                {notice ? (
                  <div
                    className={[
                      "inline-flex w-full max-w-sm items-center gap-2 rounded-2xl px-4 py-3 text-sm",
                      notice.type === "success"
                  ...
                  <div className="w-full max-w-sm rounded-2xl bg-rose-500/10 px-4 py-3 text-sm text-rose-200">
                    {error}
                  </div>
                )}
              </div>
