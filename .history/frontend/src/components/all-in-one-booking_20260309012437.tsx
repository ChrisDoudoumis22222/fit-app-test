...
const generateGoogleCalendarUrl = (booking: {
  ...
  const formatDateTime = (y: number, m: number, d: number, h: number, min: number) =>
    `${y}${String(m).padStart(2, "0")}${String(d).padStart(2, "0")}T${String(h).padStart(
      2,
      "0"
      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative w-full max-w-[520px] overflow-hidden rounded-[30px] border border-white/10 bg-zinc-950/95 shadow-[0_24px_70px_rgba(0,0,0,0.82)]"
          >
            ...
            <div className="relative z-10 max-h-[calc(100dvh-24px)] overflow-y-auto px-4 py-5 sm:px-6 sm:py-6">
              <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-3xl sm:mb-5">
                <div
                  className={cn(
              ...
              <h2
                ...
                className="text-center text-2xl font-bold leading-tight text-white sm:text-[30px]"
              >
                {status === "success" ? "Η κράτηση ολοκληρώθηκε" : "Η κράτηση απέτυχε"}
              </h2>

              <p className="mx-auto mt-2 max-w-[38ch] text-center text-sm leading-relaxed text-zinc-400 sm:text-[15px]">
                {status === "success"
                  ? "Το αίτημά σου καταχωρήθηκε επιτυχώς. Δες τα στοιχεία παρακάτω και πρόσθεσέ το εύκολα στο ημερολόγιό σου."
              ...
              {status === "success" && bookingDetails ? (
                <>
                  ...
                  <div className="mt-4 rounded-2xl border border-amber-500/15 bg-amber-500/10 p-4 sm:mt-5">
