...
export const DEFAULT_MAX_INPUT_BYTES = TEN_MB;
export const DEFAULT_HARD_MAX_BYTES = TEN_MB;
export const DEFAULT_TARGET_BYTES = Math.round(2.5 * 1024 * 1024);

export const DEFAULT_DIMENSIONS = [2560, 2200, 1920, 1680, 1440, 1280, 1080, 960, 840];
export const DEFAULT_QUALITIES = [0.96, 0.93, 0.9, 0.87, 0.84, 0.8, 0.76];

export const FILE_INPUT_ACCEPT =
  ".png,.jpg,.jpeg,.webp,.pdf,image/png,image/jpeg,image/webp,application/pdf";

export const CAMERA_INPUT_ACCEPT =
...
export const getFileExt = (fileOrName) => {
  const name =
...
export function getAcceptedFileKind(file) {
...
export async function prepareCompressedUploadFile(
  file,
  {
  ...
  if (file.size > maxInputBytes) {
    throw new Error(
      `Το αρχείο είναι πολύ μεγάλο. Μέγιστο επιτρεπτό μέγεθος: ${fmtBytes(maxInputBytes)}.`
    );
  }
  ...
  const handlePickedFile = useCallback(
    async (file) => {
      ...
      try {
        ...
        const result = await prepareCompressedUploadFile(file, {
          maxInputBytes,
          hardMaxBytes,
          targetBytes,
          alwaysReencode,
