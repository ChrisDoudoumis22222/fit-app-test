...
export default function TrainerQuickBook({
  ...
  const scrollToField = (sectionKey, focusKey) => {
    const sectionRef = getRefByKey(sectionKey)
    ...
    pulseHighlight(sectionKey, focusKey)

    if (focusKey) {
      window.setTimeout(() => {
        const focusRef = getRefByKey(focusKey)
        focusRef?.current?.focus?.({ preventScroll: true })
      }, 260)
    }
  }

  const sectionGlow = (key) =>
    highlighted[key]
      ? "ring-2 ring-white/12 bg-white/[0.03] shadow-[0_0_0_1px_rgba(255,255,255,.03),0_14px_34px_rgba(255,255,255,.04)]"
      : ""

  const inputGlow = (key) =>
    highlighted[key]
      ? "border-white/35 bg-white/[0.06] ring-2 ring-white/10"
      : "border-zinc-700/80 bg-zinc-950/60"

  const inputGlowMobile = (key) =>
    highlighted[key]
      ? "border-white/35 bg-white/[0.07] ring-2 ring-white/10"
  ...
  const validateAndGuide = () => {
    ...
    if (!cleanEmail && !cleanUsername) {
      setErr("Βάλε τουλάχιστον email ή όνομα πελάτη.")
      scrollToField("contact", "username")
      return false
    }

    if (cleanEmail && !isValidEmail(cleanEmail)) {
      setErr("Το email δεν είναι έγκυρο.")
      scrollToField("contact", "email")
      return false
    }
    ...
    if (!sched) {
      setErr("Δεν βρέθηκε διαθέσιμο ωράριο.")
      scrollToField("top")
      return false
    }
    ...
    if (!selectedSlot || selectedSlot.status !== "available") {
      setErr("Η ώρα δεν είναι διαθέσιμη πλέον. Διάλεξε άλλη.")
      scrollToField("slots")
      return false
    }
    ...
    if (pendingIntervals.some((p) => overlaps(selectedStart, endMin, p.s, p.e))) {
      setErr("Η ώρα είναι ήδη pending. Διάλεξε άλλη.")
      scrollToField("slots")
      return false
    }
    ...
    <div className="hidden sm:block w-full h-full">
      ...
      <div className="px-0 py-0 h-full flex flex-col">
        <div className="flex flex-col gap-4 flex-1 overflow-y-auto pr-1">
          <div
            ref={desktopTopRef}
            className={`border-b border-zinc-800/90 pb-4 rounded-[24px] transition-all duration-300 ${sectionGlow(
              "top"
            )}`}
          >
          ...
          <Tile
            ref={desktopSlotsRef}
            className={`p-0 rounded-[24px] transition-all duration-300 ${sectionGlow("slots")}`}
          >
          ...
          <Tile
            ...
            className={`p-0 rounded-[24px] transition-all duration-300 ${sectionGlow(
              "contact"
            )}`}
          >
            <div className="space-y-3">
              <label className="block">
                ...
                <input
                  ...
                  className={`mt-1.5 h-11 w-full rounded-2xl border px-3 text-base text-white placeholder-white/30 transition focus:outline-none focus:ring-2 focus:ring-zinc-600/40 ${inputGlow(
                    "email"
                  )}`}
                  value={email}
                  onChange={(e) => {
                    setEmail(e.target.value)
                    clearHighlight("contact", "email")
                  }}
                />
              ...
              <label className="block">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    clearHighlight("contact", "username")
                  }}
                />
                <p className="mt-1.5 text-xs text-white/45">
          ...
          <div className="absolute inset-x-0 bottom-0">
            <div className="qb-sheet-in w-full rounded-t-[30px] border border-zinc-900 bg-black shadow-[0_-30px_80px_rgba(0,0,0,.9)]">
              ...
              <div className="qb-scroll max-h-[92dvh] overflow-y-auto bg-black px-0 pb-[env(safe-area-inset-bottom)] text-white">
                <div
                  ref={mobileTopRef}
                  className={`border-b border-zinc-800/90 bg-black px-4 pt-3 pb-4 rounded-[24px] transition-all duration-300 ${sectionGlow(
                    "top"
                  )}`}
                >
                  <div className="flex items-start gap-3">
                ...
                <div className="space-y-4 bg-black px-4 py-4 text-white">
                  ...
                  <Tile
                    ref={mobileSlotsRef}
                    className={`p-0 text-white rounded-[24px] transition-all duration-300 ${sectionGlow(
                      "slots"
                    )}`}
                  >
                    <div className="space-y-3">
                    ...
                    <div className="space-y-3">
                      <label className="block">
                        ...
                        <input
                          ...
                          type="email"
                          placeholder="client@example.com"
                          className={`mt-2 h-12 w-full rounded-2xl border px-3 text-base text-white placeholder-white/50 transition focus:outline-none focus:ring-2 focus:ring-zinc-600/40 ${inputGlowMobile(
                            "email"
                          )}`}
                          value={email}
                          onChange={(e) => {
