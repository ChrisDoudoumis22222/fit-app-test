...
export default function UserBookingDetailsDock({
  ...
  useEffect(() => {
    ...
    const hasEverything =
      incomingBooking &&
      safeId(incomingBooking) === id &&
      incomingBooking.date &&
      incomingBooking.start_time &&
      (incomingBooking.trainer_name || incomingBooking.trainer?.full_name);

    if (hasEverything) {
      setFull(incomingBooking);
      setLoading(false);
      setErr(null);
      return;
    }
  ...
  const durationLabel = `${full?.duration_min ?? 0} λεπτά`;
  const timeLabel = `${hhmm(full?.start_time)} – ${hhmm(full?.end_time)}`;

  const trainerOnlineLink = useMemo(
    () => normalizeUrl(full?.trainer?.online_link || ""),
  ...
  }, [full?.trainer?.offline_location, full?.trainer?.location]);
...
{String(paymentStatus).toLowerCase() !== "απλήρωτο" && (
                  ...
                  <div>
                    ...
                    <ul className="space-y-3 text-gray-300">
                      <Li
                        ...
                        label="Όνομα"
                        value={
                          full?.trainer_name || full?.trainer?.full_name || "—"
                        }
                      />
                      {!!full?.trainer?.email && (
                        <Li
                      ...
                      {!!full?.trainer?.offline_location && !full?.is_online && (
                        <Li
                          icon={MapPin}
                          label="Τοποθεσία"
                          value={full?.trainer?.offline_location}
                        />
                      )}
                      {!!full?.trainer?.online_link && !!full?.is_online && (
                        <Li
                          icon={LinkIcon}
                          label="Online link"
                          value={getDisplayUrl(full?.trainer?.online_link)}
                        />
                      )}
                    </ul>
                  </div>
