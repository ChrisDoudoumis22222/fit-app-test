...
export default function UserBookingDetailsDock({
  ...
  const calendarUrl = useMemo(() => {
    ...
    const title = `Peak Velocity • Προπόνηση με ${trainerName}`;
    ...
    const details = [
      ...
      `Ώρα: ${hhmm(full?.start_time)} – ${
        hhmm(full?.end_time) || hhmm(end.toTimeString()?.slice(0, 5))
      }`,
    ...
    const locationValue = full?.is_online
      ? "Online συνεδρία"
      : "Peak Velocity";
  ...
  const handleAddToCalendar = () => {
    if (!calendarUrl || typeof window === "undefined") return;
