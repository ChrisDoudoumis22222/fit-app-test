...
function StatusGlow({ status }) {
  const glowColors = {
    pending: {
      outer: "bg-amber-400/20",
      middle: "bg-amber-400/40",
      inner: "bg-amber-400/80",
      core: "bg-amber-300",
      shadow: "shadow-[0_0_60px_20px_rgba(251,191,36,0.3)]",
    },
    accepted: {
      outer: "bg-emerald-400/20",
      middle: "bg-emerald-400/40",
      inner: "bg-emerald-400/80",
      core: "bg-emerald-300",
      shadow: "shadow-[0_0_60px_20px_rgba(52,211,153,0.3)]",
    },
    declined: {
      outer: "bg-rose-400/20",
      middle: "bg-rose-400/40",
      inner: "bg-rose-400/80",
      core: "bg-rose-300",
      shadow: "shadow-[0_0_60px_20px_rgba(251,113,133,0.3)]",
    },
  };

  const colors = glowColors[status] || glowColors.pending;
    ...
    <div className="pointer-events-none absolute -right-8 -top-8 z-10">
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div
            className={[
              "absolute h-32 w-32 rounded-full blur-3xl transition-colors duration-500",
              colors.outer,
            ].join(" ")}
          />
          {/* Middle glow layer */}
          <div
            className={[
              "absolute left-4 top-4 h-24 w-24 rounded-full blur-2xl transition-colors duration-500",
              colors.middle,
            ].join(" ")}
          />
          {/* Inner glow layer */}
          <div
            className={[
              "absolute left-8 top-8 h-16 w-16 rounded-full blur-xl transition-colors duration-500",
              colors.inner,
            ].join(" ")}
          />
          {/* Core bright spot */}
          <div
            className={[
              "absolute left-12 top-12 h-8 w-8 rounded-full blur-md transition-colors duration-500",
              colors.core,
              colors.shadow,
            ].join(" ")}
          />
          ...
          <motion.div
            ...
            className={[
              "absolute left-10 top-10 h-12 w-12 rounded-full transition-colors duration-500",
              colors.middle,
            ].join(" ")}
          />
        </motion.div>
...
function MobileSheet({ onClose, children, status }) {
  return (
...
function DesktopModal({ onClose, children, status }) {
  return (
      ...
      <div className="fixed inset-0 z-[120] overflow-y-auto">
        <div className="flex min-h-full items-start justify-center px-4 py-6 lg:px-6 lg:py-8">
          <motion.div
            ...
            transition={{ duration: 0.24, ease: [0.22, 1, 0.36, 1] }}
            className="relative w-full max-w-2xl overflow-hidden rounded-[30px] bg-[#06080c] p-5 shadow-[0_24px_70px_rgba(0,0,0,0.55)]"
          >
            {/* Status glow indicator */}
            <StatusGlow status={status} />
...
export default function BookingModal({
  ...
  return createPortal(
    <AnimatePresence>
      {open && (
        <>
          <MobileSheet onClose={onClose} status={localStatus}>{content}</MobileSheet>
          <DesktopModal onClose={onClose} status={localStatus}>{content}</DesktopModal>
        </>
      )}
    </AnimatePresence>,
    document.body
