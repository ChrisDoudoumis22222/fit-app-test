  ...
  return (
    <div className="fixed inset-0 z-[130]">
      ...
      <div className="absolute inset-0 flex items-center justify-center px-4">
        <div className="w-full max-w-md rounded-[26px] border border-zinc-800 bg-black p-5 shadow-[0_24px_80px_rgba(0,0,0,0.88)]">
...
function ActionBtn({
  ...
  const styles = {
    decline:
      "border-zinc-700 bg-zinc-800 text-white/85 hover:bg-zinc-700",
    accept:
    ...
    <div className="fixed inset-0 z-[130]">
      ...
      <div className="absolute inset-0 flex items-center justify-center px-4">
        <motion.div
          ...
          className={[
            "w-full max-w-md rounded-[26px] border bg-black p-5 shadow-[0_24px_80px_rgba(0,0,0,0.88)]",
            tone.ring,
          ].join(" ")}
        >
          <div className="mb-4 flex items-start justify-between gap-3">
            <div className="flex min-w-0 items-start gap-3">
              <div
                className={[
                  "grid h-11 w-11 shrink-0 place-items-center rounded-[16px] border border-zinc-700",
                  tone.iconBg,
                  tone.iconText,
                ].join(" ")}
              >
            ...
            <button
              type="button"
              onClick={loading ? undefined : onClose}
              className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-zinc-700 bg-zinc-800 text-white/75 transition hover:bg-zinc-700 hover:text-white disabled:pointer-events-none disabled:opacity-40"
              disabled={loading}
              aria-label="Κλείσιμο"
            >
              <X className="h-4 w-4" />
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              ...
              onClick={onClose}
              disabled={loading}
              className="inline-flex min-h-[52px] items-center justify-center rounded-[18px] border border-zinc-700 bg-zinc-800 px-4 py-3 text-[15px] font-semibold text-white/90 transition hover:bg-zinc-700 disabled:cursor-not-allowed disabled:opacity-45"
            >
              Άκυρο
            </button>
            ...
            <button
              ...
              disabled={loading}
              className={[
                "inline-flex min-h-[52px] items-center justify-center gap-2 rounded-[18px] border px-4 py-3 text-[15px] font-semibold transition disabled:cursor-not-allowed disabled:opacity-45",
                tone.button,
              ].join(" ")}
            >
              {loading ? (
    ...
    default: function AcceptDialog(props) {
      return (
        <BaseConfirmDialog
          ...
          description="Θέλεις να εγκρίνεις αυτή την κράτηση; Μόλις γίνει αποδοχή, τα κουμπιά θα εξαφανιστούν."
    ...
    default: function DeclineDialog(props) {
      return (
        <BaseConfirmDialog
          ...
          description="Θέλεις να απορρίψεις αυτή την κράτηση; Μόλις γίνει απόρριψη, τα κουμπιά θα εξαφανιστούν."
...
function BookingModalContentImpl({
  ...
  const showActions = isPending && canManageStatus;
      ...
      <AnimatePresence initial={false}>
        {showActions ? (
          ...
          >
            ...
            >
              {actionLoading === "declined" ? "Γίνεται απόρριψη..." : "Απόρριψη"}
            </ActionBtn>
            ...
            <ActionBtn
              ...
              disabled={Boolean(actionLoading)}
            >
              {actionLoading === "accepted" ? "Γίνεται αποδοχή..." : "Αποδοχή"}
            </ActionBtn>
          </motion.div>
        ) : null}
      </AnimatePresence>
...
export default function BookingModal({
  ...
  useEffect(() => {
    const next = normalizeStatus(localBooking?.status);
    setLocalStatus((prev) => {
  ...
  const handleStatusUpdate = useCallback(
    async (nextStatus) => {
      ...
      try {
        ...
        setLocalStatus(normalizeStatus(updatedBooking.status ?? nextStatus));
        setStatusAnimKey((n) => n + 1);
        setShowAcceptDialog(false);
        setShowDeclineDialog(false);

        if (typeof onDone === "function") {
          await onDone(updatedBooking, normalizeStatus(updatedBooking.status));
        }
      } catch (error) {
        setErrorMessage(
