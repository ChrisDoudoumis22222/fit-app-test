...
import SuccessMessage from "../quickbook/SuccessMessage"
import ErrorMessage from "../quickbook/ErrorMessage"
import QuickBookDatePicker from "../quickbook/QuickBookDatePicker"

/* ---------------- small ui helpers ---------------- */
const Tile = React.forwardRef(({ className = "", children }, ref) => (
...
const InlineError = ({ message }) =>
  message ? <p className="mt-2 text-xs text-red-300/95">{message}</p> : null
...
const normalizePickerDate = (value) => {
  ...
  if (typeof value === "object") {
    if (value?.target?.value) return normalizePickerDate(value.target.value)
    if (value?.date) return normalizePickerDate(value.date)
    if (value?.value) return normalizePickerDate(value.value)
    if (value?.selected) return normalizePickerDate(value.selected)
  }
  return ""
}
...
const isBookedStatus = (status) =>
  ["accepted", "booked", "confirmed", "completed", "paid", "approved"].includes(
    normalizeStatus(status)
  )
...
async function readScheduleFromDB(trainerId, isoDate, hints) {
  ...
  try {
    ...
    const onHoliday = (holidayRows || []).some(
      (h) => isoDate >= h.starts_on && isoDate <= h.ends_on
    )
    ...
    if (onHoliday) {
      return {
        ...
        breaks: [],
        stepMin: 15,
        sessionMin: hints?.sessionMinutes || 60,
        breakMin: 0,
        defaultOnline: false,
      }
    }
  ...
  try {
    const { data: settings } = await supabase
      ...
      .select("slot_minutes,break_minutes")
      .eq("trainer_id", trainerId)
      .maybeSingle()

    if (settings) {
      sessionMin = Number(settings.slot_minutes ?? sessionMin) || sessionMin
      breakMin = Number(settings.break_minutes ?? 0) || 0
    }
  } catch (_) {}
...
function bookingsToHiddenBookedIntervals(rows) {
  return (rows || [])
    ...
    .map((r) => {
      const s =
        strToMinutes(r.start_time) - Number(r.break_before_min || 0)
      const rawEnd = r.end_time
        ? strToMinutes(r.end_time)
        : strToMinutes(r.start_time) + Number(r.duration_min || 0)
      const e = rawEnd + Number(r.break_after_min || 0)
      return { s, e }
    })
}
...
function buildRenderableSlots(baseWindows, breaks, booked, pending, durationMin, stepMin) {
  ...
  baseWindows.forEach((w) => {
    for (let s = w.s; s + durationMin <= w.e; s += stepMin) {
      ...
      const isPending =
        !isRest && !isBooked && (pending || []).some((p) => overlaps(s, e, p.s, p.e))
...
export default function TrainerQuickBook({
  ...
  const desktopDateRef = useRef(null)
  const desktopSlotsRef = useRef(null)
  const desktopEmailWrapRef = useRef(null)
  const desktopUsernameWrapRef = useRef(null)
  ...
  const mobileSlotsRef = useRef(null)
  const mobileEmailWrapRef = useRef(null)
  const mobileUsernameWrapRef = useRef(null)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const { data, error } = await supabase
        ...
        .select(
          "id,start_time,end_time,duration_min,status,break_before_min,break_after_min"
        )
        .eq("trainer_id", trainerId)
        .eq("date", date)
  ...
  const bookedIntervals = useMemo(() => bookingsToHiddenBookedIntervals(dayRows), [dayRows])
  const pendingIntervals = useMemo(() => bookingsToPendingIntervals(dayRows), [dayRows])
  ...
  const selectedOpenSlot = useMemo(
    () => slots.find((x) => x.s === selectedStart && x.status === "open"),
    [slots, selectedStart]
  )

  const canSubmit = useMemo(() => {
    if (!trainerId || !sched) return false
    if (!cleanEmail && !cleanUsername) return false
    if (cleanEmail && !isValidEmail(cleanEmail)) return false
    if (selectedStart == null) return false
    return !!selectedOpenSlot
  }, [trainerId, sched, cleanEmail, cleanUsername, selectedStart, selectedOpenSlot])

  const isMobileViewport = () =>
    typeof window !== "undefined" &&
    window.matchMedia("(max-width: 639.98px)").matches

  const getFieldRefByKey = (key) => {
    ...
    const refs = mobile
      ? {
          top: mobileTopRef,
          date: mobileDateRef,
          slots: mobileSlotsRef,
          email: mobileEmailWrapRef,
          username: mobileUsernameWrapRef,
        }
      : {
          top: desktopTopRef,
          date: desktopDateRef,
          slots: desktopSlotsRef,
          email: desktopEmailWrapRef,
          username: desktopUsernameWrapRef,
        }

    return refs[key]
  }

  const getFocusRefByKey = (key) => {
    const mobile = isMobileViewport()

    const refs = mobile
      ? {
          email: mobileEmailRef,
          username: mobileUsernameRef,
        }
      : {
          email: desktopEmailRef,
          username: desktopUsernameRef,
        }

    return refs[key]
  }

  const pulseHighlight = (...keys) => {
    ...
    validKeys.forEach((k) => {
      if (highlightTimers.current[k]) clearTimeout(highlightTimers.current[k])
      highlightTimers.current[k] = setTimeout(() => {
        setHighlighted((prev) => ({ ...prev, [k]: false }))
      }, 1800)
    })
  }

  const clearHighlight = (...keys) => {
  ...
  const scrollToField = (fieldKey, focusKey) => {
    const fieldRef = getFieldRefByKey(fieldKey)
    ...
    pulseHighlight(fieldKey, focusKey)

    if (focusKey) {
      window.setTimeout(() => {
        const focusRef = getFocusRefByKey(focusKey)
  ...
  const fieldWrapClass = (key) =>
    cn(
      "rounded-[22px] p-2 transition-all duration-300",
      highlighted[key] ? "bg-white/[0.035] ring-1 ring-white/12" : ""
    )

  const inputClass = ({ key, mobile = false, hasError = false }) =>
    cn(
      mobile ? "mt-2 h-12" : "mt-1.5 h-11",
      "w-full rounded-2xl border px-3 text-base text-white placeholder-white/30 transition focus:outline-none focus:ring-2 focus:ring-zinc-600/40",
      hasError
        ? "border-red-400/28 bg-red-500/[0.05] ring-1 ring-red-400/18"
        : highlighted[key]
        ? mobile
          ? "border-white/30 bg-white/[0.06] ring-1 ring-white/10"
          : "border-white/30 bg-white/[0.05] ring-1 ring-white/10"
        : mobile
        ? "border-zinc-700/80 bg-zinc-950"
        : "border-zinc-700/80 bg-zinc-950/60"
    )

  const dateButtonClass = ({ mobile = false, hasError = false }) =>
    cn(
      mobile ? "h-12" : "h-11",
      "w-full rounded-2xl border px-3 text-left text-base text-white transition",
      hasError
        ? "border-red-400/28 bg-red-500/[0.05] ring-1 ring-red-400/18"
        : highlighted.date
        ? "border-white/30 bg-white/[0.05] ring-1 ring-white/10"
  ...
  const handleDateChange = (value) => {
    ...
    setDate(nextDate)
    setSelectedStart(null)
    clearHighlight("date", "slots")
    clearFieldErrors("date", "slots")
  }

  const validateAndGuide = () => {
    setOkMsg("")
    setErr(null)

    if (!trainerId) {
      setErr("Λείπει το trainerId.")
      scrollToField("top")
      return false
    }
    ...
    if (!date) {
      nextErrors.date = "Επίλεξε ημερομηνία."
    }

    if (!cleanEmail && !cleanUsername) {
      nextErrors.email = "Βάλε τουλάχιστον email ή όνομα."
      nextErrors.username = "Βάλε τουλάχιστον email ή όνομα."
    } else if (cleanEmail && !isValidEmail(cleanEmail)) {
      nextErrors.email = "Το email δεν είναι έγκυρο."
    }

    if (!sched || !(sched.windows || []).length) {
      nextErrors.date = "Δεν υπάρχει πρόγραμμα για αυτή τη μέρα."
    }

    if (selectedStart == null) {
      nextErrors.slots = "Επίλεξε ώρα."
    } else {
      ...
      if (!selectedSlot || selectedSlot.status !== "open") {
        nextErrors.slots = "Η ώρα δεν είναι διαθέσιμη πλέον. Διάλεξε άλλη."
      } else if (bookedIntervals.some((b) => overlaps(selectedStart, endMin, b.s, b.e))) {
        nextErrors.slots = "Η ώρα δεν είναι διαθέσιμη πλέον. Διάλεξε άλλη."
      } else if (
        pendingIntervals.some((p) => overlaps(selectedStart, endMin, p.s, p.e))
      ) {
        nextErrors.slots = "Η ώρα είναι ήδη pending. Διάλεξε άλλη."
      }
    }
    ...
    if (nextErrors.email && nextErrors.username) {
      pulseHighlight("email", "username")
      scrollToField("email", "email")
      return false
    }
    ...
    if (nextErrors.username) {
      scrollToField("username", "username")
      return false
    }
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        break_before_min: 0,
        break_after_min: Number(sched?.breakMin || 0),
        is_online: isOnline,
        note: note.trim() || null,
        status: "accepted",
        amount: hasAmount ? numericAmount : 0,
      }
      ...
      setFieldErrors({})
      setHighlighted({})
      setCreatedPayload(inserted || payload)
  ...
  const submitBtnClassDesktop = saving
    ? "border-zinc-700/80 bg-zinc-800 text-white cursor-wait"
    : canSubmit
    ? "border-white bg-white text-black hover:bg-zinc-100"
    : "border-zinc-700/80 bg-zinc-800 text-white/68 hover:bg-zinc-700"

  const submitBtnClassMobile = saving
    ? "border-zinc-700/80 bg-zinc-800 text-white cursor-wait"
    : canSubmit
    ? "border-white bg-white text-black hover:bg-zinc-100"
    : "border-zinc-700/80 bg-zinc-800 text-white/68 hover:bg-zinc-700"
    ...
    <div className="hidden sm:block w-full h-full">
      ...
      <div className="px-0 py-0 h-full flex flex-col">
        <div className="flex flex-col gap-4 flex-1 overflow-y-auto pr-1">
          <div
            ref={desktopTopRef}
            className="border-b border-zinc-800/90 pb-4 rounded-[24px]"
          >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  ...
                  <span className="inline-flex items-center rounded-full border border-emerald-500/15 bg-emerald-500/12 px-2.5 py-1 text-[11px] text-emerald-200/95">
                    {loadingDay || !sched ? "…" : `${openCount} ώρες`}
                  </span>
          ...
          <Tile className="p-0">
            <div className="space-y-4">
              <div ref={desktopDateRef} className={fieldWrapClass("date")}>
          ...
          <Tile
            ref={desktopSlotsRef}
            className={cn("p-0 rounded-[24px]", fieldWrapClass("slots"))}
          >
            <div className="space-y-3">
                ...
                <div className="bg-transparent py-1">
                  ...
                  >
                    {slots.map(({ s, e, status }) => {
                      ...
                      const disabled = saving || status !== "open"

                      let cls =
                        "h-11 rounded-2xl border px-2.5 text-center text-sm font-medium transition"

                      if (status === "pending") {
                        cls += " border-yellow-500/30 bg-yellow-500/18 text-yellow-100 cursor-not-allowed"
                      } else if (isSel) {
                        cls += " border-emerald-400/30 bg-emerald-500/25 text-emerald-50 shadow-[0_8px_18px_rgba(16,185,129,.18)]"
                      } else {
                        cls += " border-emerald-500/20 bg-emerald-500/14 text-emerald-100/95 hover:bg-emerald-500/18"
                      }
              ...
              <InlineError message={fieldErrors.slots} />
            </div>
          </Tile>

          <Tile className="p-0">
            <div className="space-y-3">
              <label ref={desktopEmailWrapRef} className={cn("block", fieldWrapClass("email"))}>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    clearFieldErrors("email", "username")
                  }}
                />
              ...
              <label
                ref={desktopUsernameWrapRef}
                className={cn("block", fieldWrapClass("username"))}
              >
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    clearFieldErrors("username", "email")
                  }}
                />
                <p className="mt-1.5 text-xs text-white/45">
              ...
              <label className="block">
                <FieldLabel icon={FileText}>Σημείωση / Παρατήρηση (προαιρετικό)</FieldLabel>
                <textarea
                  rows={3}
          ...
          <div className="sticky bottom-0 z-10 border-t border-zinc-800/90 bg-[rgba(12,12,15,.92)] pt-3 pb-4 backdrop-blur-xl">
            <div className="flex flex-col gap-3">
              ...
              <button
                ...
                disabled={saving}
                className={`h-11 w-full rounded-2xl border transition disabled:opacity-90 text-[15px] font-medium ${submitBtnClassDesktop}`}
              >
          ...
          <div className="absolute inset-x-0 bottom-0">
            <div className="qb-sheet-in w-full rounded-t-[30px] border border-zinc-900 bg-black shadow-[0_-30px_80px_rgba(0,0,0,.9)]">
              ...
              <div className="qb-scroll max-h-[92dvh] overflow-y-auto bg-black px-0 pb-[env(safe-area-inset-bottom)] text-white">
                <div ref={mobileTopRef} className="border-b border-zinc-800/90 bg-black px-4 pt-3 pb-4">
                  <div className="flex items-start gap-3">
                ...
                <div className="space-y-4 bg-black px-4 py-4 text-white">
                  <Tile className="p-0 text-white">
                    <div className="space-y-4">
                      <div ref={mobileDateRef} className={fieldWrapClass("date")}>
                  ...
                  <Tile ref={mobileSlotsRef} className={cn("p-0 text-white", fieldWrapClass("slots"))}>
                    <div className="space-y-3">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            <Clock className="h-4 w-4 shrink-0 text-white/80" />
                            <p className="text-[15px] font-semibold text-white">Ώρες ({uiSession}′)</p>
                          </div>
                      ...
                      <div className="flex flex-wrap items-center gap-3 text-xs text-white">
                        <span className="inline-flex items-center gap-1">
                          <span className="inline-block h-2.5 w-2.5 rounded-full bg-emerald-400/90" />
                          <span className="text-white">Ελεύθερες</span>
                        </span>
                        <span className="inline-flex items-center gap-1">
                          <span className="inline-block h-2.5 w-2.5 rounded-full bg-yellow-400/90" />
                      ...
                      {!sched ? (
                        ...
                        <div className="text-sm text-white">Ρεπό / χωρίς ωράριο για αυτή τη μέρα.</div>
                      ) : (
                        <div className="bg-transparent">
                          <div className="qb-scroll max-h-[30dvh] overflow-y-auto pr-1">
                            {loadingDay ? (
                              <Spinner label="Φόρτωση ωρών…" />
                            ) : (
                              <div className="grid grid-cols-3 gap-2">
                                {slots.map(({ s, e, status }) => {
                                  ...
                                  if (status === "pending") {
                                    ...
                                    cls += " border-emerald-500/20 bg-emerald-500/14 text-emerald-100 hover:bg-emerald-500/18"
                                  }
                  ...
                  <Tile className="p-0 text-white">
                    <div className="space-y-3">
                      <label
                        ref={mobileEmailWrapRef}
                        className={cn("block", fieldWrapClass("email"))}
                      >
                      ...
                      <label className="block">
                        <FieldLabel icon={FileText} className="text-white">
                          Σημείωση / Παρατήρηση (προαιρετικό)
                        </FieldLabel>
                        <textarea
                          rows={4}
                          placeholder="Οποιαδήποτε λεπτομέρεια"
                  ...
                  <div className="sticky bottom-0 z-10 -mx-4 border-t border-zinc-800/90 bg-black px-4 pt-3 pb-[calc(env(safe-area-inset-bottom)+12px)]">
                    <div className="flex flex-col gap-3">
                      ...
                      <button
                        ...
                        onClick={handleCreate}
                        disabled={saving}
                        className={`h-12 w-full rounded-2xl border transition disabled:opacity-90 text-[15px] font-semibold ${submitBtnClassMobile}`}
                      >
                        <span className="inline-flex items-center justify-center gap-2">
                          {saving ? (
                            <>
  ...
  return (
    <>
      <ErrorMessage message={err} onClose={() => setErr(null)} />

      <SuccessMessage
        message={okMsg}
        onClose={() => {
          const payloadToSend = createdPayload
          setOkMsg("")
          setCreatedPayload(null)
          onCreated && onCreated(payloadToSend)
        }}
        calendarEvent={
          createdPayload
            ? {
                title: "Peak Velocity Booking",
                description: `Κράτηση ${createdPayload.is_online ? "Online" : "Δια ζώσης"}`,
                location: createdPayload.is_online ? "Online" : "Studio / Gym",
                start: `${createdPayload.date}T${createdPayload.start_time}`,
                end: `${createdPayload.date}T${createdPayload.end_time}`,
                filename: "peak-velocity-booking.ics",
              }
            : null
        }
      />
