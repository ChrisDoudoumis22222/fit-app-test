...
function statusMeta(status) {
  ...
  if (key === "accepted") {
    return {
      ...
      banner:
        "border-emerald-500/30 bg-emerald-500/12 text-emerald-200 shadow-[0_10px_30px_rgba(16,185,129,0.08)]",
      iconWrap:
        "bg-emerald-500/20 text-emerald-300 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]",
      accent:
        "from-emerald-500/30 via-emerald-400/18 to-transparent border-emerald-400/20",
      accentGlow: "bg-emerald-400/18",
      accentLine: "from-emerald-400/80 to-emerald-400/10",
    };
  }
  ...
  if (key === "declined") {
    return {
      ...
      banner:
        "border-rose-500/30 bg-rose-500/12 text-rose-200 shadow-[0_10px_30px_rgba(244,63,94,0.08)]",
      iconWrap:
        "bg-rose-500/20 text-rose-300 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]",
      accent:
        "from-rose-500/30 via-rose-400/18 to-transparent border-rose-400/20",
      accentGlow: "bg-rose-400/18",
      accentLine: "from-rose-400/80 to-rose-400/10",
    };
  }
  ...
  return {
    ...
    banner:
      "border-amber-500/25 bg-amber-500/[0.10] text-amber-200 shadow-[0_10px_30px_rgba(245,158,11,0.06)]",
    iconWrap:
      "bg-amber-500/18 text-amber-300 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]",
    accent:
      "from-amber-500/28 via-amber-400/16 to-transparent border-amber-400/20",
    accentGlow: "bg-amber-400/15",
    accentLine: "from-amber-400/70 to-amber-400/10",
  };
}
...
function ActionBtn({
  ...
  const styles = {
    decline:
      "border-rose-500/45 bg-rose-500/12 text-rose-300 hover:bg-rose-500/18 hover:border-rose-500/60",
    accept:
      "border-emerald-500/45 bg-emerald-500/12 text-emerald-300 hover:bg-emerald-500/18 hover:border-emerald-500/60",
  ...
  return (
    <AnimatePresence mode="wait" initial={false}>
      <motion.div
        ...
        initial={{ opacity: 0, scale: 0.92, x: 18, y: -18 }}
        animate={{ opacity: 1, scale: 1, x: 0, y: 0 }}
        exit={{ opacity: 0, scale: 0.92, x: 18, y: -18 }}
        transition={{ duration: 0.28, ease: [0.22, 1, 0.36, 1] }}
        className="pointer-events-none absolute right-0 top-0 z-0 h-28 w-28 overflow-hidden rounded-bl-[28px] rounded-tr-[26px]"
      >
        <div
          className={[
            "absolute inset-0 border-l border-b bg-gradient-to-bl",
            meta.accent,
          ].join(" ")}
        />
        <div
          className={[
            "absolute right-5 top-5 h-12 w-12 rounded-full blur-2xl",
            meta.accentGlow,
          ].join(" ")}
        />
        <div
          className={[
            "absolute right-0 top-0 h-[2px] w-20 bg-gradient-to-l",
            meta.accentLine,
          ].join(" ")}
        />
        <div
          className={[
            "absolute right-0 top-0 h-20 w-[2px] bg-gradient-to-b",
            meta.accentLine,
          ].join(" ")}
        />
      </motion.div>
    ...
    <div className="mb-4 relative z-[1]">
      <div className="flex items-start justify-between gap-3">
        ...
        <button
          type="button"
          onClick={onClose}
          className="relative z-[2] grid h-11 w-11 shrink-0 place-items-center rounded-full bg-white/[0.04] text-white/75 transition hover:bg-white/[0.08] hover:text-white"
          aria-label="Κλείσιμο"
        >
          <X className="h-5 w-5" />
        </button>
