...
function useFloatingPanel(triggerRef, open, { width = 320, gap = 8 } = {}) {
...
function WheelColumn({
  ...
  return (
    <div className="rounded-[24px] border border-white/10 bg-white/[0.025] p-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] backdrop-blur-xl">
      <div className="mb-2.5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          <button
            type="button"
            onClick={onIncrement}
            className="flex h-8 w-8 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] text-white/90 transition hover:bg-white/[0.08] hover:text-white"
          >
      ...
      <div className="relative">
        <div
          className="pointer-events-none absolute left-0 right-0 z-[3] rounded-[18px] border border-white/10 bg-[linear-gradient(180deg,rgba(115,115,125,0.22),rgba(75,75,85,0.18))] shadow-[0_8px_30px_rgba(0,0,0,.22),inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-md"
          style={{
            top: CENTER_OFFSET,
            height: ITEM_HEIGHT,
          }}
        ...
        <div className="pointer-events-none absolute inset-x-0 top-0 z-[4] h-10 rounded-t-[18px] bg-gradient-to-b from-zinc-950 via-zinc-950/88 to-transparent" />
        <div className="pointer-events-none absolute inset-x-0 bottom-0 z-[4] h-10 rounded-b-[18px] bg-gradient-to-t from-zinc-950 via-zinc-950/88 to-transparent" />
          ...
          {loopedItems.map((item, index) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "flex w-full snap-center items-center justify-center text-base font-semibold tracking-[0.02em] transition-all duration-150",
                  selected ? "text-white" : "text-white/85 hover:text-white"
                )}
...
export default function QuickBookTimePicker({
  ...
  const scrollWheelToValue = useCallback((ref, items, selected, behavior = "smooth") => {
    const el = ref.current;
    if (!el) return;

    const baseIndex = items.indexOf(selected);
    const safeIndex = baseIndex >= 0 ? baseIndex : 0;
    const middleIndex = items.length + safeIndex;
  ...
  const snapLoopWheel = useCallback((ref, items, onPicked) => {
    ...
    const middleIndex = items.length + normalizedIndex;
  ...
  const nudgeHour = (delta) => {
    setDraftHour((prev) => {
      const next = hourOptions[normalizeLoopIndex(prev + delta, hourOptions.length)];
  ...
  const nudgeMinute = (delta) => {
    setDraftMinute((prev) => {
      const currentIndex = minuteOptions.indexOf(prev);
      const safeIndex = currentIndex >= 0 ? currentIndex : 0;
      const nextIndex = normalizeLoopIndex(safeIndex + delta, minuteOptions.length);
    ...
    <div className={cn("relative z-[20]", className)}>
      <style>{`
        .timepicker-scroll {
          ...
          scrollbar-color: rgba(170,170,180,.42) transparent;
        }
      ...
      >
        ...
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.04] text-white transition group-hover:bg-white/[0.06] group-hover:text-white">
            ...
            <>
              ...
              <motion.div
                ...
                className={cn(
                  ...
                  isMobile
                    ? "z-[2000] mx-auto max-w-[400px] rounded-[24px]"
                    : "z-[2000] rounded-[24px]"
                )}
              >
                ...
                <div className="p-3.5 sm:p-4">
                  ...
                  <div className="mt-3.5 rounded-[22px] border border-white/10 bg-white/[0.025] px-4 py-3 backdrop-blur-md">
                    <div className="text-[10px] uppercase tracking-[0.15em] text-white/65">
                      Preview
                    </div>
                ...
                <div className="flex items-center justify-between gap-3 border-t border-white/10 px-3.5 py-3 sm:px-4">
                  <button
                    type="button"
                    onClick={handleNow}
                    className="rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-2.5 text-sm font-medium text-white transition hover:bg-white/[0.08] hover:text-white"
                  >
                    Τώρα
                  </button>
