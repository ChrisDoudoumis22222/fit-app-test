...
function buildCalendarTitle(event) {
  ...
  if (event?.trainerName) {
    return `Προπόνηση με ${event.trainerName} μέσω του Peak Velocity`
  }

  if (event?.serviceName) {
    return `${event.serviceName} μέσω του Peak Velocity`
  }

  return "Κράτηση μέσω του Peak Velocity"
}

function buildCalendarLocation(event) {
  if (event?.location) return event.location
  if (event?.isOnline) return "Online συνεδρία"
...
function buildCalendarDescription(event) {
  ...
  const lines = [
    "Στοιχεία κράτησης από το Peak Velocity",
    "",
    `Κωδικός κράτησης: ${safeValue(event?.bookingId)}`,
    `Όνομα πελάτη: ${safeValue(event?.userName)}`,
    `Email πελάτη: ${safeValue(event?.userEmail)}`,
    `Trainer: ${safeValue(event?.trainerName)}`,
    `Υπηρεσία: ${safeValue(event?.serviceName)}`,
    `Τρόπος συνεδρίας: ${event?.isOnline ? "Online" : "Δια ζώσης"}`,
    `Τοποθεσία: ${buildCalendarLocation(event)}`,
    `Κατάσταση: ${safeValue(event?.status, "Επιβεβαιωμένη")}`,
    `Πληρωμή: ${safeValue(event?.paymentMethod)}`,
    `Ποσό: ${priceText}`,
    `Σημειώσεις: ${safeValue(event?.note)}`,
  ]

  if (event?.description) {
    lines.push("", "Πρόσθετες πληροφορίες:", event.description)
  }
...
function downloadICS(event) {
  ...
  const ics = [
    ...
    `DTSTART:${start}`,
    `DTEND:${end}`,
    `SUMMARY:${escapeICS(buildCalendarTitle(event))}`,
    `DESCRIPTION:${escapeICS(buildCalendarDescription(event))}`,
    `LOCATION:${escapeICS(buildCalendarLocation(event))}`,
    "STATUS:CONFIRMED",
    "END:VEVENT",
    "END:VCALENDAR",
  ].join("\r\n")
  ...
  a.href = url
  a.download = event.filename || "kratisi-peak-velocity.ics"
      ...
      <div className="absolute inset-0 flex items-center justify-center p-4 sm:p-6">
        ...
        >
          ...
          <button
            type="button"
            onClick={onClose}
            aria-label="Κλείσιμο μηνύματος επιτυχίας"
            className="
          ...
          <div className="relative z-10 px-5 pb-6 pt-8 sm:px-7 sm:pb-7 sm:pt-9">
            <div className="flex flex-col items-center text-center">
              <div className="relative flex h-[108px] w-[108px] items-center justify-center sm:h-[118px] sm:w-[118px]">
                ...
                <div className="absolute flex h-[50px] w-[50px] items-center justify-center rounded-full bg-emerald-500 text-[24px] shadow-[0_8px_24px_rgba(16,185,129,0.28)]">
                  🎉
                </div>
              </div>
