...
function buildLocalizedCalendarEvent(event = {}) {
  ...
  const title = trainerName
    ? `Προπόνηση με τον/την ${trainerName} στις ${startTime} μέσω του Peak Velocity`
    : `Προπόνηση στις ${startTime} μέσω του Peak Velocity`;

  const descriptionLines = [
    "Η κράτηση καταχωρήθηκε επιτυχώς μέσω του Peak Velocity.",
    ...
    event.isOnline ? "Τύπος: Online προπόνηση" : "",
    event.location ? `Τοποθεσία: ${event.location}` : "",
    event.note ? `Σημειώσεις: ${event.note}` : "",
  ].filter(Boolean);
  ...
  return {
    ...
    location:
      event.location || (event.isOnline ? "Online προπόνηση" : "Peak Velocity"),
