...
function buildCalendarTitle(event) {
  return (
    event.title ||
    (event.serviceName
      ? `Κράτηση Peak Velocity • ${event.serviceName}`
      : "Κράτηση Peak Velocity")
  )
}
...
function buildCalendarDescription(event) {
  const lines = [
    "Στοιχεία κράτησης Peak Velocity",
    "",
    `Κωδικός κράτησης: ${safeValue(event.bookingId)}`,
    `Υπηρεσία: ${safeValue(event.serviceName)}`,
    `Trainer / Πάροχος: ${safeValue(event.trainerName)}`,
    `Πελάτης: ${safeValue(event.userName)}`,
...
export default function SuccessMessage({
  ...
  subtitle = "Η ενέργεια αποθηκεύτηκε με επιτυχία",
  calendarEvent = null,
}) {
