...
function buildLocalizedCalendarEvent(event = {}) {
  ...
  const descriptionLines = [
    "Η κράτησή σου καταχωρήθηκε επιτυχώς μέσω του Peak Velocity.",
  ...
  return {
    ...
    location: event.location || (event.isOnline ? "Online προπόνηση" : "Peak Velocity"),
...
function downloadICS(event) {
  ...
  const ics = [
    ...
    `DTSTART:${start}`,
    `DTEND:${end}`,
    `SUMMARY:${escapeICS(localizedEvent.title || "Προπόνηση μέσω του Peak Velocity")}`,
...
export default function SuccessMessage({
  ...
  const handleAddToCalendar = async () => {
    ...
    try {
      ...
      // Για Apple συσκευές, το πιο reliable από browser είναι .ics import
      const isAppleDevice =
