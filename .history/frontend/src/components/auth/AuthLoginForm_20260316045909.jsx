...
export default function AuthLoginForm({
  ...
  accountStatus = "",
  accountStatusType = "info", // "info" | "success" | "warning"
}) {
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      {checkingAccountStatus ? (
        <StatusBox
          ...
          message="Γίνεται έλεγχος λογαριασμού και onboarding..."
        />
      ) : accountStatus ? (
        <StatusBox
...
function StatusBox({ type = "info", icon, message }) {
  const styles =
    type === "success"
  ...
  return (
    <div
      className={`rounded-2xl border px-4 py-3 text-sm flex items-start gap-2 ${styles}`}
    >
