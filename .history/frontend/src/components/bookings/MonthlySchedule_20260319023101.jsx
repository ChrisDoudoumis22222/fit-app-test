...
function BookingPill({ b, onOpen, compact = false }) {
  ...
  const endCls = compact ? "text-[10px] text-white/60" : "text-[11px] text-white/60";
...
function EmptyDayState({ mobile = false }) {
  return (
    <div
      className={[
        "flex items-center gap-2 rounded-xl border border-dashed border-white/10 bg-white/[.03]",
        mobile ? "px-3 py-3" : "px-2.5 py-2.5",
      ].join(" ")}
    >
      <PencilLine className={mobile ? "h-4 w-4 text-white/40" : "h-3.5 w-3.5 text-white/40"} />
      <span className={mobile ? "text-[12px] text-white/50" : "text-[11px] text-white/45"}>
...
function DayCountBadge({ count }) {
  return (
    <span className="inline-flex items-center justify-center min-w-[22px] h-6 px-2 rounded-full border border-white/10 bg-white/[.08] text-[11px] font-semibold text-white/75">
      {count}
    </span>
  );
}

function DesktopMonthCell({ d, anchor, dayRows, onOpenDetails, onSelectDate, loading, error }) {
  ...
  return (
    <div
      ...
      className={[
        ...
        "min-h-[190px]",
        inMonth
          ? "bg-[rgba(17,18,21,.55)] border-white/10"
          : "bg-[rgba(17,18,21,.28)] border-white/8 opacity-75",
        today ? "ring-1 ring-red-400/40" : "",
    ...
    >
      ...
      {loading ? (
        <div className="h-[120px] rounded-xl bg-white/[.06] animate-pulse border border-white/10" />
      ) : error ? (
        ...
        <div
          className="
            space-y-2 max-h-[128px] overflow-y-auto pr-1
...
function MobileDayCard({ d, dayRows, onOpenDetails, onSelectDate, loading, error }) {
  const key = toYMDLocal(d);
  const today = isToday(d);
  ...
  return (
    <div
      ...
      className={[
        ...
        "bg-[rgba(17,18,21,.68)] border-white/10 backdrop-blur-xl",
        "shadow-[inset_0_1px_0_rgba(255,255,255,.04),0_12px_28px_rgba(0,0,0,.38)]",
        today ? "ring-1 ring-red-400/40" : "",
      ].join(" ")}
    >
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            {today && (
              <span className="rounded-full border border-red-400/20 bg-red-500/15 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-red-200">
                Σήμερα
              </span>
            )}
          </div>
        ...
        <div className="shrink-0">
          <DayCountBadge count={loading || error ? "—" : dayRows.length} />
        </div>
      </div>

      {loading ? (
        <div className="space-y-2">
          <div className="h-16 rounded-2xl bg-white/[.06] animate-pulse border border-white/10" />
          <div className="h-16 rounded-2xl bg-white/[.04] animate-pulse border border-white/10" />
        </div>
      ) : error ? (
...
export default function MonthlySchedule({
  ...
  return <Glass className="p-3 sm:p-6">{content}</Glass>;
}