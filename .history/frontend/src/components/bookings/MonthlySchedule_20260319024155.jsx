...
function BookingPill({ b, onOpen, compact = false }) {
  ...
  const metaCls = compact ? "text-[10px] text-white/60" : "text-[11px] text-white/60";
...
function EmptyDayState({ mobile = false }) {
  return (
    <div
      className={[
        "flex items-center gap-2 rounded-xl bg-white/[.03]",
        mobile ? "px-3.5 py-3.5" : "px-3 py-3",
      ].join(" ")}
    >
      <PencilLine
        className={
          mobile ? "h-4.5 w-4.5 text-white/40" : "h-4 w-4 text-white/40"
        }
      />
      <span
        className={
          mobile ? "text-[12px] text-white/50" : "text-[11px] text-white/45"
        }
      >
...
function DesktopMonthCell({
  ...
  const DESKTOP_VISIBLE_BOOKINGS = 3;
  const visibleRows = dayRows.slice(0, DESKTOP_VISIBLE_BOOKINGS);
  const hiddenCount = Math.max(dayRows.length - DESKTOP_VISIBLE_BOOKINGS, 0);

  return (
    <div
      onClick={() => onSelectDate?.(key)}
      className={[
        "rounded-[22px] p-3 h-[260px] overflow-hidden",
        inMonth
          ? "bg-[rgba(17,18,21,.55)]"
    ...
    >
      ...
      {loading ? (
        ...
        <div className="space-y-2.5">
          ...
          {hiddenCount > 0 && (
            <div className="px-1 pt-0.5 text-[11px] font-medium text-white/55">
              +{hiddenCount} ακόμα
            </div>
          )}
        </div>
