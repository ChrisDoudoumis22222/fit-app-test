  ...
  return {
    ...
    trainer_info: {
      full_name:
        ...
        null,
      email:
        data?.trainer_info?.email ||
        data?.trainer?.email ||
        null,
      avatar_url:
        data?.trainer_info?.avatar_url ||
        data?.trainer?.avatar_url ||
        null,
    },
  };
}
    ...
    <div className="pointer-events-none absolute inset-0 rounded-3xl overflow-hidden">
      ...
      {accepted && (
        <motion.div
          ...
          transition={{ duration: 0.35, ease: "easeOut" }}
          className="absolute inset-x-0 top-0 h-20 bg-gradient-to-b from-emerald-400/10 to-transparent"
        />
      )}
    </div>
...
const Li = ({ icon: Icon, label, value }) => (
  <li className="flex items-start gap-3">
    <Icon className="h-5 w-5 sm:h-6 sm:w-6 text-gray-500 mt-0.5 shrink-0" />
...
function MobileShell({ children, onClose }) {
  return (
    <>
      <div className="fixed inset-0 z-[90] bg-black/70 backdrop-blur-sm sm:hidden" />
      <motion.div
      ...
      >
        <div className="h-full w-full overflow-y-auto bg-[#0b0c0f] px-4 pb-6 pt-[max(1rem,env(safe-area-inset-top))]">
          <div className="sticky top-0 z-10 mb-4 flex items-center justify-between border-b border-white/10 bg-[#0b0c0f]/95 px-1 py-3 backdrop-blur">
...
export default function BookingDetailsDock({
  ...
  const content = (
    <Glass
      ...
      className="p-4 sm:p-5 h-fit sm:block"
    >
      ...
      {!id ? null : loading ? (
        ...
        <>
          <motion.div
            ...
            transition={{ duration: 0.35, ease: "easeOut" }}
            className="flex items-center gap-4 mb-6 rounded-2xl border p-3 sm:p-4"
          >
            {clientAvatar ? (
              <div className="relative shrink-0">
                <img
                  ...
                  className={`h-14 w-14 sm:h-16 sm:w-16 rounded-full object-cover border shadow-[0_8px_24px_rgba(0,0,0,.35)] ${
                    status === "accepted"
              ...
              <div
                className={`grid h-14 w-14 sm:h-16 sm:w-16 place-items-center rounded-full border shadow-[0_8px_24px_rgba(0,0,0,.35)] shrink-0 ${
                  status === "accepted"
                    ? "bg-emerald-500/10 border-emerald-300/20 ring-2 ring-emerald-300/10"
                    : "bg-white/8 border-white/10 ring-2 ring-white/10"
                }`}
              >
            ...
            <div className="min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-2 mt-2">
                <motion.span
                  ...
                  transition={{ duration: 0.35, ease: "easeOut" }}
                  className={`inline-flex px-2.5 py-0.5 rounded-lg text-xs ${statusTone}`}
                >
                  {statusLabel}
                </motion.span>
                ...
                {status === "accepted" && (
                  <motion.span
                    ...
                    className="inline-flex items-center gap-1 rounded-lg bg-emerald-500/12 px-2.5 py-0.5 text-xs text-emerald-300 border border-emerald-400/10"
                  >
    ...
    <>
      ...
      <div className="sm:hidden">
        <AnimatePresence mode="wait">
          <MobileShell onClose={onClose}>{content}</MobileShell>
        </AnimatePresence>
      </div>
