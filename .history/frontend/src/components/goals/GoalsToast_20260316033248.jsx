...
export default function GoalsToast({ toast, onClose }) {
  ...
  useEffect(() => {
    ...
    const duration = toast.duration ?? (isMobile ? 5000 : 3500);
  ...
  const autoCloseDuration = toast?.duration ?? (isMobile ? 5000 : 3500);
      ...
      {isOpen && (
        <motion.div
          ...
          className={cn(
            ...
            "flex items-end justify-center px-4 pt-4 sm:items-start sm:justify-end sm:p-4"
          )}
          style={{
          ...
          >
            ...
            {!isLoading && (
              <motion.div
                ...
                className={cn(
                  "absolute bottom-0 left-0 hidden h-[2px] w-full origin-left sm:block",
