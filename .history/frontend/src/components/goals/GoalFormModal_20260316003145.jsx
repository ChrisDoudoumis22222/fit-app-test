    ...
    <FieldShell label={label} icon={icon} helper={helper}>
      <div className="relative">
        <select
          ...
          disabled={disabled}
          className={cn(
            "w-full appearance-none rounded-2xl border border-white/10 bg-zinc-900/60 px-4 py-3 pr-11 text-zinc-100 backdrop-blur-xl transition-all duration-200 focus:border-white/30 focus:outline-none focus:ring-2 focus:ring-white/20",
...
export default function GoalFormModal({ open, onClose, initial, onSubmit }) {
  ...
  const numericTarget = useMemo(
    () => Math.max(0, Number(form.target_value) || 0),
    [form.target_value]
  );

  const numericProgress = useMemo(
    () => Math.max(0, Number(form.progress_value) || 0),
    [form.progress_value]
  );

  const reachedGoal = useMemo(() => {
    return numericTarget > 0 && numericProgress >= numericTarget;
  }, [numericTarget, numericProgress]);

  useEffect(() => {
    setForm((prev) => {
      const target = Math.max(0, Number(prev.target_value) || 0);
      const progress = Math.max(0, Number(prev.progress_value) || 0);
      const hasReachedGoal = target > 0 && progress >= target;

      if (hasReachedGoal && prev.status !== "completed") {
  ...
  }, [form.target_value, form.progress_value]);
  ...
  const availableStatusOptions = useMemo(() => {
    if (reachedGoal) return statusOptions;
  ...
  const handleSubmitAction = async () => {
    ...
    try {
      const targetValue = Math.max(0, Number(form.target_value) || 0);
      const progressValue = Math.max(0, Number(form.progress_value) || 0);
      const autoCompleted = targetValue > 0 && progressValue >= targetValue;

      const normalizedStatus = autoCompleted
        ? "completed"
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-4">
              ...
              <SelectField
                ...
                icon={
                  <StatusIcon
                    className={cn("h-3.5 w-3.5", selectedStatus?.color)}
                  />
                }
                value={form.status}
                ...
                helper={
                  reachedGoal
                    ? "Ο στόχος ολοκληρώθηκε αυτόματα επειδή η πρόοδος έφτασε τον στόχο."
                    : "Το 'Ολοκληρώθηκε' ενεργοποιείται αυτόματα μόνο όταν η πρόοδος φτάσει τον στόχο."
                }
              />
            </div>
