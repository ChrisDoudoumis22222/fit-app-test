...
export default function NavbarNewsCard() {
  ...
  const redCards = useMemo(() => {
    ...
    return cards;
  }, [missingAvatar, missingDiploma, dismissed.avatar, dismissed.diploma]);

  const yellowCard = useMemo(() => {
    if (!hasPendingBookings || dismissed.pending) return null;
  ...
  const greenCard = useMemo(() => {
    if (dismissed.tip) return null;
    return selectedTip;
  }, [dismissed.tip, selectedTip]);
  ...
  const activeCard = useMemo(() => {
    ...
    return null;
  }, [redCards, yellowCard, greenCard]);

  if (loading || !activeCard) return null;
      ...
      <AnimatePresence mode="wait" initial={false}>
        ...
        >
          <NewsCard
            ...
            onClose={() => handleDismiss(activeCard.key)}
          />
        </motion.div>
      </AnimatePresence>
    </div>
