...
type TrainerPricingRow = {
  ...
  package_discounts: Record<string, any>;
  specialty_pricing: Record<string, any>;
  updated_at: string;
...
export default function QuickBookModal({
  ...
  const [savedBookingPrice, setSavedBookingPrice] = useState<number | null>(null);
  ...
  const loadPricing = useCallback(async () => {
    ...
    try {
      ...
      setPricing((data as TrainerPricingRow | null) || null);
    } catch (e) {
...
const submitBooking = async () => {
  ...
  if (!pricing) {
    setBanner({
      ...
      message: "Ο προπονητής δεν έχει ορίσει ακόμη τιμές στο trainer_pricing.",
    });
    return;
  }
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("book_trainer_slot", {
      ...
      p_is_online: !!selectedSlot.is_online,
    ...
    // if your DB trigger/RPC already sets amount + currency_code correctly,
    // you can delete this whole update block.
    const { error: amountSaveError } = await supabase
    ...
    if (amountSaveError) throw amountSaveError;
    ...
    setSavedBookingPrice(finalBookingPrice);
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span>
            ...
            <strong className="font-semibold">
              {priceLoading ? "Φόρτωση..." : fmtPrice(bookingPrice, currencyCode)}
            </strong>
          </span>
        </div>
      </div>
