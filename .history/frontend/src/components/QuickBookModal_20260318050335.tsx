...
export default function QuickBookModal({
    ...
    try {
      ...
      if (trainer?.profile_id?.trim()) {
        const { data, error } = await supabase
          ...
          .eq("trainer_id", trainer.profile_id.trim())
          .maybeSingle();
      ...
      if (!row && trainerIdCandidates.length) {
        ...
        if (error) throw error;
        row = Array.isArray(data) && data.length > 0 ? (data[0] as TrainerPricingRow) : null;
      }
  ...
  }, [open, trainer, trainerIdCandidates]);
  ...
  const submitBooking = async () => {
    ...
    try {
      ...
      setSavedBookingPrice(safeAmount);
    ...
    <div>
      ...
      {!priceLoading && !pricingExists && (
        <div className="mb-4 flex items-center gap-2 rounded-2xl border border-amber-400/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
          <AlertTriangle className="h-4 w-4" />
          {pricingMessage ||
            "Ο προπονητής δεν έχει ορίσει ακόμη τιμή. Η κράτηση μπορεί να σταλεί κανονικά."}
        </div>
      )}
