...
const TrainerOnboardingPage = lazy(() => import("./pages/TrainerOnboardingPage"));
...
function Shell({ children }) {
  return (
    <div className="lg:pl-[calc(var(--side-w)+4px)] transition-[padding]">
      {children}
    </div>
  );
}
...
function UserOnboardingRedirect() {
      ...
      <BrowserRouter>
        <Suspense fallback={<Loading />}>
          <Routes>
            ...
            {/* temporary user onboarding compatibility */}
            <Route
              path="/user/onboarding"
              element={
                <ProtectedRoute>
                  <UserOnboardingRedirect />
                </ProtectedRoute>
              }
            />
            <Route
