...
export default function PostDetailPage() {
  ...
  const fetchPost = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await withTimeout(q, 12000, "Timeout: αργεί πολύ η φόρτωση της ανάρτησης")
      if (error) {
        setError(error.message)
      } else {
        // compute trainer rating
        let trainerData = data.trainer ?? {}
        if (trainerData?.id) {
          try {
            const { data: revs } = await withTimeout(
              supabase.from("trainer_reviews").select("rating").eq("trainer_id", trainerData.id),
              8000
            )
            const ratings = (revs ?? []).map((r) => Number(r.rating)).filter((n) => !Number.isNaN(n))
  ...
  // initial + refetch on id/profile change
  useEffect(() => {
    fetchPost()
  }, [fetchPost])

  // stall watchdog: after 15s of (authLoading || loading) show stalled UI
  useEffect(() => {
  ...
  const handleRetry = () => { setStalled(false); fetchPost() }
        ...
        <div className="flex items-center justify-center h-[75vh]">
          {stalled ? (
            <motion.div
              ...
              className="text-center p-8 rounded-3xl max-w-md border border-amber-500/30 bg-black/40 backdrop-blur-xl shadow-[0_8px_32px_rgba(245,158,11,0.2)]"
            >
              <AlertCircle className="h-12 w-12 text-amber-400 mx-auto mb-4" />
              <h3 className="text-lg font-semibold text-amber-300 mb-2">Άργησε πολύ να φορτώσει</h3>
              <p className="text-amber-200/90 mb-6">Ξεκίνα ξανά ή δοκίμασε να επαναφορτώσεις τη σελίδα.</p>
        ...
        <div className="flex items-center justify-center h-[75vh]">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                onClick={handleRetry}
                className="flex-1 px-6 py-3 bg-red-600/20 text-red-200 rounded-xl hover:bg-red-600/30 transition border border-red-500/30"
              >
                Δοκίμασε ξανά
              </button>
              <button
