...
// Enhanced cache class for better performance
class DataCache {
  constructor() {
    this.cache = new Map()
...
// Memoized background component - UPDATED TO REMOVE WHITE PARTS
...
const useTrainerData = (profile) => {
  ...
  const cacheKey = useMemo(() => profile?.id || "no-profile", [profile?.id])
  ...
  }, [profile?.id, cacheKey, fetchTrainerData])

  return {
    performanceData,
    loading,
    error,
    refetch: useCallback(() => fetchTrainerData(true), [fetchTrainerData]),
  }
}

// Memoized Trainer Performance Stats component
...
function TrainerDashboard() {
  ...
  // Memoized time update
  useEffect(() => {
    const timer = setInterval(() => setCurrentTime(new Date()), 1000)
    return () => clearInterval(timer)
  }, [])

  // Memoized profile sync
  useEffect(() => {
    if (!profile) return
  ...
  // Memoized profile data for header
  ...
  // Get current category data - memoized
  const currentCategory = useMemo(
