...
// ⤵️ external search + filters UI
import TrainerSearchNav from "../components/TrainerSearchNav.tsx";

// ⤵️ lazy-load your booking popup component (create it in ../components/QuickBookModal.tsx)
...
const TrainerCard = memo(function TrainerCard({
  ...
  // Helper: should a click be ignored (inside a button/link/input etc.)?
  ...
  const handleCardClick = (e) => {
    // if the user clicked an interactive control, don't navigate
    if (isInteractiveTarget(e.target)) return;
    goProfile();
  };

  const handleKeyDown = (e) => {
    // allow keyboard access: Enter / Space
    if (isInteractiveTarget(e.target)) return;
  ...
  return (
    ...
    >
      {/* Image (fixed height so all tiles align) */}
      ...
      {/* Content column – make it a flex column so actions stick to the bottom */}
      <div className={`${list ? "flex-1 p-5" : "p-4"} flex flex-col`}>
        ...
        {/* Expandable area (adds height only when open) */}
...
export default function ServicesMarketplacePage() {
  ...
  // booking popup (external component)
  ...
  const buildProfilesQuery = useCallback(() => {
    ...
    if (selectedCity !== "all") {
      ...
      if (city) {
        const tokens = [city.value, ...city.aliases].map((t) => t.replace(/,/g, ""));
        const ors = tokens.map((t) => `ilike(location,%${t}%)`).join(",");
        if (ors) q = q.or(ors);
      }
    }
    ...
    return q;
  }, [catFilter, onlyOnline, searchTerm, selectedCity, sortBy]);
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      let added = 0;
      let extraPagesTried = 0;
      let localNew = [];

      while (added < PAGE_SIZE && (hasMore || extraPagesTried === 0) && extraPagesTried <= MAX_EXTRA_PAGES_PER_LOAD) {
        const from = (page + extraPagesTried) * PAGE_SIZE;
        ...
        const existingIds = new Set([...items.map((t) => t.id), ...localNew.map((t) => t.id)]);
        const uniqueToAdd = filteredBatch.filter((t) => !existingIds.has(t.id));

        localNew = [...localNew, ...uniqueToAdd];
      ...
      setPage((prev) => prev + Math.max(1, extraPagesTried));
    } catch {
  ...
  }, [applyClientFilters, buildProfilesQuery, hasMore, hydrateTrainers, items, loadingPage, page]);
            ...
            <main className="mx-auto w-full px-0 sm:px-6 lg:px-12 xl:px-16 pt-0 sm:pt-6 lg:pt-10 pb-[80px] lg:pb-[100px]">
              ...
              {hasMore && !loadingPage && (
                <div className="mt-10 flex justify-center">
                  <PremiumButton variant="secondary" onClick={loadMore}>Φόρτωσε περισσότερα</PremiumButton>
                </div>
              )}
            </main>
          </div>
        ...
        {bookingTrainer && (
          ...
          >
            <QuickBookModal
              ...
              // Optional: onBooked={(info) => { setBookingTrainer(null); navigate(`/trainer/${bookingTrainer.id}`); }}
            />
          </Suspense>
        )}
      </AnimatePresence>
