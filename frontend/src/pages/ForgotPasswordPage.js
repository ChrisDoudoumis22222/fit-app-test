/* ForgotPasswordPage.jsx – glassy UI to match Auth page */
"use client";
...
export default function ForgotPasswordPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setState({
        msg: "✅ Έλεγξε το inbox σου για τον σύνδεσμο επαναφοράς.",
        type: "success",
        loading: false,
      });
    } catch (err) {
...
function SubmitButton({ disabled }) {
  return (
    ...
    >
      {disabled ? <Loader2 className="animate-spin w-5 h-5" /> : "Στείλε σύνδεσμο επαναφοράς"}
    </motion.button>
  );
}
