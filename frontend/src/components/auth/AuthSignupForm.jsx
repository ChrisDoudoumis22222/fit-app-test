...
export default function AuthSignupForm({
  ...
  const selectedCategory = useMemo(() => {
    const base = categoryByValue(form.category);
    if (!base) return null;

    return {
      ...base,
      specialties: SPECIALTIES_BY_VALUE[base.value] || [],
    };
  }, [form.category]);

  const showGoogleSignupButton = true;
  const googleSignupText =
    form.role === "trainer"
      ? "Εγγραφή trainer με Google"
      : "Εγγραφή με Google";
              ...
              pushToast?.(
                nextRole === "trainer"
                  ? {
                      ...
                      message:
                        "Μπορείς να κάνεις trainer εγγραφή και με Google. Αν μπεις με Google, το πρώτο βήμα του trainer wizard θα ζητήσει όνομα και περιοχή.",
                      durationMs: 8000,
                    }
                  : {
                      type: "info",
...
function CategorySelect({ categories = [], value, onChange }) {
  const cat = categoryByValue(value);
