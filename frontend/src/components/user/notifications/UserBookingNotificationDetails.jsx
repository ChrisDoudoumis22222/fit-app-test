...
function buildGoogleCalendarEvent(booking) {
  ...
  const trainerName = getDisplayName(booking);
  ...
  return {
    title: `${bookingTitle} • ${trainerName}`,
    details,
    location,
    start,
    end,
...
function buildGoogleCalendarIOSUrl(event) {
  if (!event) return "";
...
function buildICSContent(event) {
  ...
  return [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Peak Velocity//Booking Calendar//EN",
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH",
    "BEGIN:VEVENT",
    `UID:${uid}`,
...
export default function UserBookingNotificationDetails({
  open,
  booking,
  onClose,
}) {
  ...
  const durationMinutes = useMemo(() => getDurationMinutes(booking), [booking]);
  ...
  const calendarEvent = useMemo(
    () => buildGoogleCalendarEvent(booking),
    [booking]
  );
  ...
  const handleAddToCalendar = useCallback(() => {
    ...
    if (os === "ios") {
      const iosUrl = buildGoogleCalendarIOSUrl(calendarEvent);
      openWithFallback(iosUrl, () => {
        downloadICSFile(calendarEvent);
      });
      return;
    }
  ...
  const bookingTitle = getBookingTitle(booking);
  const notes = getBookingNotes(booking);
  const price = formatCurrency(getPrice(booking));
  const start = hhmm(booking?.start_time);
  const end = hhmm(booking?.end_time);
  const sessionType = booking?.is_online ? "Online" : "Δια ζώσης";
  const dateValue = formatDate(booking?.date || booking?.booking_date);
    ...
    <div className="fixed inset-0 z-[9999]">
      ...
      >
        <div className="flex min-h-full items-start justify-center sm:pt-4">
          <Glass className="relative w-full sm:max-w-2xl">
  ...
  <X className="h-5 w-5 sm:h-5 sm:w-5" />
            ...
            <div className="px-4 py-4 sm:px-5 sm:py-5">
              ...
              <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-2">
                ...
                <InfoTile
                  icon={booking?.is_online ? Wifi : MapPin}
                  label="Τύπος"
                  value={sessionType}
                />
                <InfoTile
                  ...
                  label="Κόστος"
                  value={price}
                  tone={getPrice(booking) ? "success" : "default"}
                />
              </div>
              ...
              {notes ? (
                <div className="mb-4 rounded-2xl border border-white/10 bg-white/[.04] p-4 backdrop-blur-xl">
                  <div className="mb-2 flex items-center gap-2 text-white/55">
                    ...
                    <span className="text-xs">Σημειώσεις</span>
                  </div>
