...
import { Loader2, Trash2, CheckCircle2, AlertTriangle, X } from "lucide-react";
...
export default function AvatarCard({
  ...
  // ✅ allow local placeholders too ("/images/..." etc.)
  const placeholder = useMemo(() => {
    if (typeof placeholderUrl === "string" && placeholderUrl.trim()) return placeholderUrl.trim();
  ...
  // ✅ Upload ONLY to storage here; let parent update DB via onUpload(url)
  const uploadAvatar = useCallback(
    async (e) => {
      ...
      if (file.size > MAX_INPUT_BYTES) {
        ...
        flash("error", "Το αρχείο είναι πολύ μεγάλο.");
      ...
      try {
        ...
        // ✅ overwrite same key per user (no orphan files, fewer ops)
        ...
        flash(
          "success",
          hitTarget ? `Έγινε! Συμπίεση σε ${fmtBytes(blob.size)}.` : `Έγινε! (best) ${fmtBytes(blob.size)}.`
        );

        // ✅ parent handles DB update (prevents double updates)
        if (typeof onUpload === "function") {
          await onUpload(finalUrl);
        } else {
          // fallback: update profile if no callback given
    ...
    [bucket, onUpload, flash]
  );

  // ✅ Delete from storage here; parent handles DB via onDelete() (if provided)
  ...
  }, [hasCustomAvatar, avatarUrl, bucket, placeholder, onDelete, flash]);

  const primaryLabel = uploading ? "Ανέβασμα…" : hasCustomAvatar ? "Αλλαγή εικόνας" : "Επιλογή εικόνας";
        ...
        <div className="relative px-6 pb-6 sm:px-8 sm:pb-8 pt-8">
          <div className="mx-auto max-w-xl flex flex-col items-center text-center">
            ...
            <div className="mt-2 text-white/45 text-xs sm:text-sm">
              PNG / JPG / WEBP • στόχος ~{fmtBytes(TARGET_BYTES)} • τελικό αρχείο ≤ {fmtBytes(HARD_MAX_BYTES)}
            </div>
