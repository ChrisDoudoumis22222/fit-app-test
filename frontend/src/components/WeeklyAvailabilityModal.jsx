...
function extractQuickBookTimeValue(value, fallback = "") {
  ...
  if (value && typeof value === "object") {
    if (typeof value?.target?.value === "string") {
      return normalizeTimeString(value.target.value) || fallback;
    }

    if (typeof value?.value === "string") {
      return normalizeTimeString(value.value) || fallback;
    }

    if (typeof value?.time === "string") {
      return normalizeTimeString(value.time) || fallback;
    }

    if (typeof value?.hhmm === "string") {
      return normalizeTimeString(value.hhmm) || fallback;
    }
  }
...
function SchedulerTimeField({ label, value, onChange, fallbackValue }) {
  const safeValue = normalizeTimeString(value) || fallbackValue;
  ...
  return (
    <div className="min-w-0">
      <QuickBookTimePicker
        label={label}
        value={safeValue}
        onChange={(next) => onChange(extractQuickBookTimeValue(next, safeValue))}
      />
    </div>
  );
}
...
function DayRow({ day, row, onToggle, onChange }) {
  ...
  return (
    <div className="min-w-0 rounded-2xl border border-zinc-800 bg-zinc-950/50 px-3 py-3 sm:px-4 sm:py-4 md:rounded-[26px] md:bg-zinc-950/60">
        ...
        {row.enabled ? (
          <div className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2">
            <SchedulerTimeField
              ...
              fallbackValue={DEFAULT_START}
              onChange={(nextValue) => onChange("start_time", nextValue)}
            />

            <SchedulerTimeField
              label="Ώρα λήξης"
              value={row.end_time || DEFAULT_END}
              fallbackValue={DEFAULT_END}
              onChange={(nextValue) => onChange("end_time", nextValue)}
            />
          </div>
        ) : null}
      </div>
    ...
    async function loadFromDb() {
      ...
      try {
        const [
          ...
          { data: availRows, error: availError },
        ] = await Promise.all([
          supabase
            .from("profiles")
    ...
    setRows((prev) =>
      prev.map((row) =>
        row.weekday === weekday
          ? {
              ...row,
              [field]: normalizeTimeString(value) || row[field],
            }
          : row
      )
    );
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (payload.isOnline && !/^https?:\/\//i.test(payload.online_link)) {
        throw new Error(
          "Το online link πρέπει να ξεκινά με http:// ή https://"
        );
      }
      ...
      console.error(err);
      setErrorMsg(err?.message || "Αποτυχία αποθήκευσης διαθεσιμότητας.");
    } finally {
      setSaving(false);
    }
  }
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-0 z-[1210] p-0 sm:p-4 lg:p-6"
          >
            <div
              className={cn(
                "mx-auto flex h-[100dvh] w-full flex-col overflow-hidden border border-white/10 bg-[linear-gradient(180deg,rgba(24,24,27,.98),rgba(0,0,0,.99))] shadow-[0_24px_80px_rgba(0,0,0,.55)]",
                "rounded-none sm:h-[calc(100dvh-2rem)] sm:max-w-[1100px] sm:rounded-[28px] lg:h-[calc(100dvh-3rem)]"
              )}
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-start justify-between gap-3 border-b border-white/10 px-4 pb-3 pt-[max(12px,env(safe-area-inset-top))] sm:px-5 sm:py-4">
                <div className="min-w-0">
              ...
              <div className="availability-modal-scroll min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-3 py-3 sm:px-4 sm:py-4">
                {loading ? (
                  ...
                  <section className="mx-auto w-full max-w-full">
                    <div className="rounded-[24px] border border-zinc-900 bg-zinc-950/60 p-3 sm:p-5 md:rounded-[28px]">
                      {errorMsg ? (
                      ...
                      <div className="mt-4 rounded-2xl border border-zinc-800 bg-zinc-950/50 px-3 py-3 sm:px-4 sm:py-4 md:rounded-[26px] md:bg-zinc-950/60 md:p-5">
                        <div className="mb-4 flex items-start gap-3">
                          <div className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-zinc-800 bg-zinc-900/60 text-white">
                            {globalIsOnline ? (
                        ...
                        <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
                          <button
                            ...
                            className={cn(
                              "flex min-h-[52px] w-full items-center justify-center gap-2 rounded-2xl border px-3 py-3 text-sm font-medium transition",
                              !globalIsOnline
                          ...
                          {!globalIsOnline ? (
                            <div className="rounded-2xl border border-zinc-800 bg-transparent px-4 py-3 md:bg-zinc-900/80">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setOfflineLocation(e.target.value)
                                }
                                placeholder="π.χ. Αμπελόκηποι, Αθήνα ή Οδός / Studio / Γυμναστήριο"
                                className="w-full min-w-0 rounded-xl border border-zinc-800 bg-transparent px-3 py-3 text-sm text-white placeholder:text-zinc-500 outline-none transition focus:border-white/30 md:bg-black/50"
                              />
              ...
              <div className="border-t border-white/10 px-4 pb-[max(12px,env(safe-area-inset-bottom))] pt-3 sm:px-6 sm:py-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                    <button
                      type="button"
                      onClick={onClose}
                      className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[0.04] px-5 py-3 text-sm font-semibold text-white transition hover:bg-white/10 sm:w-auto"
                    >
                      <X className="h-4 w-4" />
                      Άκυρο
                    </button>
