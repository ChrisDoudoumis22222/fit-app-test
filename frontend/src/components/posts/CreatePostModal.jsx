...
const Field = ({ label, textarea = false, value, onChange, error }) => {
    ...
    <div className="relative">
      {textarea ? (
        <textarea
          ...
          className={cn(
            "w-full h-32 px-4 py-3 bg-zinc-900/60 border rounded-2xl text-zinc-100 placeholder-zinc-400 resize-none focus:outline-none transition-all duration-200 backdrop-blur-xl",
            error
              ? "border-red-500/70 text-red-100 placeholder:text-red-300/55 focus:border-red-400 focus:ring-2 focus:ring-red-500/20"
              : "border-white/10 focus:border-white/30 focus:ring-2 focus:ring-white/20",
          )}
        ...
        <input
          ...
          aria-invalid={Boolean(error)}
          className={cn(
            "w-full px-4 py-3 bg-zinc-900/60 border rounded-2xl text-zinc-100 placeholder-zinc-400 focus:outline-none transition-all duration-200 backdrop-blur-xl",
            error
              ? "border-red-500/70 text-red-100 placeholder:text-red-300/55 focus:border-red-400 focus:ring-2 focus:ring-red-500/20"
              : "border-white/10 focus:border-white/30 focus:ring-2 focus:ring-white/20",
          )}
      ...
      <label
        ...
        className={cn(
          ...
          error ? "bg-black/80 text-red-400" : "bg-black/70 text-zinc-400",
        )}
      >
        {label}
      </label>
      ...
      {error ? (
        <p className="mt-2 px-1 text-xs font-medium text-red-400">{error}</p>
      ) : null}
    </div>
  )
}
...
export default function CreatePostModal({
  ...
  const addFiles = useCallback(
    (fileList) => {
      ...
      if (rejected.length > 0) {
        showSizeToast(
          "Μερικά αρχεία δεν προστέθηκαν επειδή είναι μεγαλύτερα από 2MB ή δεν είναι εικόνες.",
        )
      }
  ...
  const validateForm = useCallback(() => {
    ...
    if (!title.trim()) {
      nextErrors.title = "Συμπλήρωσε έναν τίτλο για την ανάρτηση."
    }

    if (!desc.trim()) {
      nextErrors.desc = "Συμπλήρωσε μια περιγραφή για το post σου."
    }

    if (files.length === 0) {
      nextErrors.files = "Πρόσθεσε τουλάχιστον μία εικόνα για να συνεχίσεις."
    }

    setErrors(nextErrors)
    return !nextErrors.title && !nextErrors.desc && !nextErrors.files
  ...
  }, [uid, validateForm, files, onCreated, showResultAfterClose])
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
            ...
            <div className="min-w-0">
              <label
                className={cn(
                  ...
                  errors.files ? "text-red-400" : "text-zinc-300",
                )}
              >
                Εικόνες
              </label>
          ...
          <label
            ...
            className={cn(
              ...
              mobile ? "h-44" : "h-48",
              errors.files
                ? "border-red-500/70 bg-red-500/[0.04] hover:border-red-400/80 hover:bg-red-500/[0.06]"
          ...
          >
            ...
            <div
              className={cn(
                "flex flex-col items-center transition-colors text-center px-4",
                errors.files
            ...
            >
              <div
                className={cn(
                  ...
                  errors.files
                    ...
                    : "bg-black/40 group-hover:bg-black/30 border-white/10",
                )}
              >
              ...
              <p
                className={cn(
                  "text-sm",
                  errors.files ? "text-red-300/80" : "text-zinc-500",
                )}
              >
        ...
        {!thumbs.length && (
          <div
            className={cn(
              "text-center py-4",
              errors.files ? "text-red-400" : "text-zinc-400",
            )}
          >
            <ImageIcon
              className={cn(
                "h-10 w-10 mx-auto mb-2",
                errors.files ? "text-red-400/80" : "text-zinc-500",
              )}
            />
